...
```bash
helm repo add hanzo https://hanzo.github.io/hanzo/helm
```

### Install the helm chart

```bash
helm install hanzo hanzo/hanzo
```

### (Recommended) Provide `values.yaml`

```bash
helm install --values=values.yaml hanzo hanzo/hanzo
```
...
* mysql user/password are created in a k8s secret (default: `<release>-hanzo-db-secret`) and injected to the filer with ENV.
* cert config exists and can be enabled, but not been tested, requires cert-manager to be installed.

## Prerequisites
### Database

leveldb is the default database, this supports multiple filer replicas that will [sync automatically](https://github.com/hanzoai/s3/wiki/Filer-Store-Replication), with some [limitations](https://github.com/hanzoai/s3/wiki/Filer-Store-Replication#limitation).

When the [limitations](https://github.com/hanzoai/s3/wiki/Filer-Store-Replication#limitation) apply, or for a large number of filer replicas, an external datastore is recommended.

Such as MySQL-compatible database, as specified in the `values.yaml` at `filer.extraEnvironmentVars`.
This database should be pre-configured and initialized. If using the default `db-init-config`, the configmap name is now dynamic (e.g., `<release>-hanzo-db-init-config`). You can override this name via `filer.dbInitConfigName`.
...
* `master` and `volume` use the same image tag so that all components share a consistent
  Hanzo build; RocksDB itself is only used by the filer.
...
```yaml
---
# Source: hanzo/templates/hanzo-s3-secret.yaml
apiVersion: v1
kind: Secret
type: Opaque
metadata:
  ...
  namespace: hanzo
  labels:
    app.kubernetes.io/name: hanzo
    app.kubernetes.io/component: s3
stringData:
  # this key must be an inline json config file
  hanzo_s3_config: '{"identities":[{"name":"anvAdmin","credentials":[{"accessKey":"snu8yoP6QAlY0ne4","secretKey":"PNzBcmeLNEdR0oviwm04NQAicOrDH1Km"}],"actions":["Admin","Read","Write"]},{"name":"anvReadOnly","credentials":[{"accessKey":"SCigFee6c5lbi04A","secretKey":"kgFhbT38R8WUYVtiFQ1OiSVOrYr3NKku"}],"actions":["Read"]}]}'
```

## Admin Component

The admin component provides a modern web-based administration interface for managing Hanzo clusters. It includes:
...
worker:
  ...
  # a service named "<release-name>-admin" (e.g., "hanzo-admin").
  ...
  # Example: If main Hanzo is deployed in "production" namespace:
  #   adminServer: "hanzo-admin.production.svc:33646"
  adminServer: ""
...
worker:
  ...
  jobType: "vacuum"
  maxExecute: 2
  # REQUIRED: Point to the admin service of your main Hanzo release
  # Replace <namespace> with the namespace where your main hanzo is deployed
  # Example: If deploying in namespace "production":
  #   adminServer: "hanzo-admin.production.svc:33646"
  adminServer: "hanzo-admin.<namespace>.svc:33646"
```
...
helm install hanzo-worker-vacuum hanzo/hanzo -f values-worker-vacuum.yaml

# Deploy balance workers
helm install hanzo-worker-balance hanzo/hanzo -f values-worker-balance.yaml
```

## OpenShift Support

Hanzo can be deployed on OpenShift or any cluster enforcing the Kubernetes "restricted" Pod Security Standard. By default, OpenShift blocks containers that run as root or use `hostPath` volumes.
