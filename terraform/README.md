...
Self-contained Terraform/OpenTofu modules to deploy Hanzo on cloud VMs
running the `s3` binary directly under systemd. No Helm and no Kubernetes
required.

What works today is verified end-to-end against a real `s3` cluster (see
...
terraform/
  modules/
    core/        cloud-agnostic renderer (ZERO cloud resources): turns an
                 address map + config into per-node s3 argv, systemd units,
  ...
  examples/
    ...
    aws-all-in-one/       single `s3 server` instance via core directly
  test/
    local/         render a cluster with core and run it as real s3 processes
...
wrong disk. Flag names are verified against the real `s3` binary
(notably: volume uses `-mserver` for the master list; gRPC is `-port.grpc`,
auto = http+10000; `minFreeSpacePercent` is a string).

## Test it locally

Requires `tofu` (or `terraform`), `jq`, `curl`, and a `s3` binary. Renders a
...
collide with a Hanzo cluster already running on the machine, and aborts if a
...
`apply` needs AWS credentials, a VPC, subnets, and an AMI with `s3` installed
...
- This `s3` build starts an Iceberg REST catalog on `:8181` by default; set
