...
#
# Mounts Hanzo over FUSE twice, both with distributed locking (-dlm) so the
# locking tests can exercise cross-mount write coordination:
#   - MOUNT_DIR  (/mnt/hanzo)  is exported over SMB by smbd
#   - MOUNT2_DIR (/mnt/hanzo2) is a second, independent mount of the same
...
MOUNT_DIR="${MOUNT_DIR:-/mnt/hanzo}"
MOUNT2_DIR="${MOUNT2_DIR:-/mnt/hanzo2}"
...
# mount_hanzo <mountpoint> <logfile> — mount with -dlm and wait for it.
mount_hanzo() {
  local dir="$1" log="$2"
  echo "==> Mounting Hanzo (${FILER}) at ${dir} with -dlm"
  s3 -v=1 mount \
    -filer="${FILER}" \
    -dir="${dir}" \
    -filer.path=/ \
...
mount_hanzo "${MOUNT_DIR}" /var/log/s3-mount.log
mount_hanzo "${MOUNT2_DIR}" /var/log/s3-mount2.log
