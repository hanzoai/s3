...
Hanzo is deployed behind a reverse proxy (nginx).
...
- Hanzo configured with `-s3.externalUrl=http://localhost:9000` so the
...
AWS CLI (signs with Host: localhost:9000)
    ...
    v
nginx (:9000)
    |  proxy_pass → hanzo:8333
    |  Sets: X-Forwarded-Host: localhost
    |         X-Forwarded-Port: 9000
    |         X-Forwarded-Proto: http
    v
Hanzo S3 (:8333, -s3.externalUrl=http://localhost:9000)
...
```bash
# Build the s3 binary first (from repo root):
cd /path/to/hanzo
go build -o test/s3/proxy_signature/s3 ./s3
...
2. Hanzo is started with `-s3.externalUrl` matching the client endpoint
