...
func TestShellUserLifecycle(t *testing.T) {
	...
	t.Run("CreateShowListDelete", func(t *testing.T) {
		...
		out := execShell(t, s3Cmd, master, filer, fmt.Sprintf("s3.user.create -name %s", userName))
	...
	t.Run("EnableDisable", func(t *testing.T) {
		...
		out := execShell(t, s3Cmd, master, filer, fmt.Sprintf("s3.user.disable -name %s", userName))
		requireContains(t, out, "disabled", "user.disable")

		out = execShell(t, s3Cmd, master, filer, fmt.Sprintf("s3.user.show -name %s", userName))
		requireContains(t, out, "disabled", "user.show after disable")

		out = execShell(t, s3Cmd, master, filer, fmt.Sprintf("s3.user.enable -name %s", userName))
	...
	t.Run("Provision", func(t *testing.T) {
		...
		execShell(t, s3Cmd, master, filer, fmt.Sprintf("s3.bucket.create -name %s", bucket2))
		out = execShell(t, s3Cmd, master, filer,
		...
		execShell(t, s3Cmd, master, filer, fmt.Sprintf("s3.bucket.delete -name %s", bucket2))
	})
}
