...
import (
	...
	"github.com/hanzoai/s3/s3/query/engine"
	flag "github.com/hanzoai/s3/s3/util/fla9"
	"github.com/hanzoai/s3/s3/util/grace"
	"github.com/hanzoai/s3/s3/util/sqlutil"
)

const usageLine = "s3-sql [-master=localhost:9333] [-interactive] [-file=query.sql] [-output=table|json|csv] [-database=dbname] [-query=\"SQL\"]"

const longHelp = `Enhanced SQL interface for Hanzo Message Queue topics with multiple execution modes.

Execution Modes:
- Interactive shell (default): s3-sql -interactive
- Single query: s3-sql -query "SELECT * FROM user_events"
- Batch from file: s3-sql -file queries.sql
- Context switching: s3-sql -database analytics -interactive
...
Examples:
  s3-sql -interactive
  s3-sql -query "SHOW DATABASES" -output json
  s3-sql -file batch_queries.sql -output csv
  s3-sql -database analytics -query "SELECT COUNT(*) FROM metrics"
  s3-sql -master broker1:9333 -interactive
`
...
// Run executes the s3-sql CLI.
func Run(args []string) int {
	fs := flag.NewFlagSet("s3-sql", flag.ContinueOnError)
	usageWriter := io.Writer(os.Stderr)
	fs.SetOutput(usageWriter)

	var opts Options
	fs.StringVar(&opts.Master, "master", "localhost:9333", "Hanzo master server HTTP address")
...
func runInteractiveShell(ctx *SQLContext) bool {
	fmt.Println("Hanzo Enhanced SQL Interface")
	...
	historyPath := path.Join(os.TempDir(), "s3-sql-history")
	...
	for {
		...
		if queryBuffer.Len() == 0 {
			if ctx.currentDatabase != "" {
				prompt = fmt.Sprintf("hanzo:%s> ", ctx.currentDatabase)
			} else {
				prompt = "hanzo> "
			}
		} else {
...
func showEnhancedHelp() {
	fmt.Println(`Hanzo Enhanced SQL Interface Help:
