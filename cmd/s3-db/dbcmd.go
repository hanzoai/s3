...
import (
	...
	"github.com/hanzoai/s3/s3/server/postgres"
	"github.com/hanzoai/s3/s3/util"
	flag "github.com/hanzoai/s3/s3/util/fla9"
)

const usageLine = "s3-db -port=5432 -master=<master_server>"

const longHelp = `Start a PostgreSQL wire protocol compatible database server that provides SQL query access to Hanzo.

This database server enables any PostgreSQL client, tool, or application to connect to Hanzo
...
Examples:
	...
	s3-db -auth=md5 -users='{"admin":"secret","readonly":"view123"}'
	
	# Start with complex passwords using JSON format
	s3-db -auth=md5 -users='{"admin":"pass;with;semicolons","user":"password:with:colons"}'
	
	# Start with credentials from JSON file (most secure)
	s3-db -auth=md5 -users="@/etc/hanzo/users.json"
	
	# Start with custom port and master
	s3-db -port=5433 -master=master1:9333
	
	# Allow connections from any host
	s3-db -host=0.0.0.0 -port=5432
	
	# Start with TLS encryption
	s3-db -tls-cert=server.crt -tls-key=server.key
...
Programming Language Examples:
	...
	# Go (lib/pq)
	db, err := sql.Open("postgres", "host=localhost port=5432 user=hanzo dbname=default sslmode=disable")
...
// Run executes the s3-db CLI.
func Run(args []string) int {
	fs := flag.NewFlagSet("s3-db", flag.ContinueOnError)
	...
	fs.StringVar(&opts.MasterAddr, "master", "localhost:9333", "Hanzo master server address")
...
func runWithOptions(opts *Options) bool {
	...
	fmt.Printf("Starting Hanzo Database Server...\n")
	fmt.Printf("Host: %s\n", opts.Host)
	fmt.Printf("Port: %d\n", opts.Port)
	fmt.Printf("Master: %s\n", opts.MasterAddr)
	...
	fmt.Printf("  psql -h %s -p %d -U hanzo -d %s\n", opts.Host, opts.Port, opts.Database)
	if len(users) > 0 {
