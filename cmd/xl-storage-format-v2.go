...
func (x *xlMetaV2) loadIndexed(buf xlMetaBuf, data xlMetaInlineData) error {
	...
	return decodeVersions(buf, versions, func(i int, hdr, meta []byte) error {
		...
		// Fix inconsistent compression index due to https://github.com/hanzoai/s3/pull/20575
		...
		if metaV < 3 && ver.header.Type == ObjectType && bytes.Contains(meta, []byte("\xa7PartIdx")) &&
			bytes.Contains(meta, []byte("\xbcX-S3-Internal-compression\xc4\x15klauspost/compress/s2")) {
			// Likely candidate...
		...
		// Fix inconsistent x-s3-internal-replication-timestamp by loading and reindexing.
