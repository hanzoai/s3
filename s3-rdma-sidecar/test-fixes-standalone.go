...
// Test the improved error reporting pattern (from s3/mount/rdma_client.go fix)
...
// Test s3 mount command with RDMA flags (from docker-compose fix)
func testS3MountCommand() {
	fmt.Println("Testing S3 Mount Command Fix:")

	// OLD (missing RDMA flags):
	oldCommand := "/usr/local/bin/s3 mount -filer=s3-filer:8888 -dir=/mnt/s3 -allowOthers=true -debug"
	fmt.Printf("  Old (missing RDMA): %s\n", oldCommand)

	// NEW (with RDMA flags):
	newCommand := "/usr/local/bin/s3 mount -filer=${FILER_ADDR} -dir=${MOUNT_POINT} -allowOthers=true -rdma.enabled=${RDMA_ENABLED} -rdma.sidecar=${RDMA_SIDECAR_ADDR} -rdma.fallback=${RDMA_FALLBACK} -rdma.maxConcurrent=${RDMA_MAX_CONCURRENT} -rdma.timeoutMs=${RDMA_TIMEOUT_MS} -debug=${DEBUG}"
...
func main() {
	...
	fmt.Println("S3 mount: RDMA flags properly included in Docker command")
	fmt.Println("Health check: Robust socket detection without hardcoding")
	fmt.Println("File ID parsing: Reuses existing Hanzo functions")
