...
echo "🚀 Hanzo RDMA Performance Benchmark"
...
benchmark_size() {
    ...
    for i in $(seq 1 $ITERATIONS); do
        ...
        local response=$(curl -s "$SIDECAR_URL/read?volume=$TEST_VOLUME&needle=$TEST_NEEDLE&cookie=$TEST_COOKIE&size=$size&volume_server=http://hanzo-volume:8080")
...
main() {
    ...
    if ! check_sidecar; then
        ...
        echo "Run: cd /path/to/s3-rdma-sidecar && docker compose -f docker-compose.mount-rdma.yml up -d"
        exit 1
    fi
