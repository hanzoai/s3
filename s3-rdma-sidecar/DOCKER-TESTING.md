...
This guide provides comprehensive Docker-based integration testing for the Hanzo RDMA sidecar system.
...
```
┌─────────────────────┐    ┌─────────────────────┐    ┌─────────────────────┐
│   Hanzo Master  │    │  Hanzo Volume   │    │    Rust RDMA       │
│     :9333          │◄──►│     :8080          │    │    Engine           │
└─────────────────────┘    └─────────────────────┘    └─────────────────────┘
                                    │                           │
                                    ▼                           ▼
...
### ✅ Core Components
- **Hanzo Master**: Cluster leadership and status
- **Hanzo Volume Server**: Volume operations and health
...
✅ Hanzo Master is ready
✅ Hanzo Master is leader and ready

🔵 Waiting for Hanzo Volume Server to be ready...
✅ Hanzo Volume Server is ready
...
./tests/docker-test-helper.sh logs hanzo-volume
```
...
### Test Hanzo Integration

```bash
# Check cluster status
curl http://hanzo-master:9333/cluster/status | jq '.'

# Check volume status  
curl http://hanzo-volume:8080/status | jq '.'

# List volumes
curl http://hanzo-master:9333/vol/status | jq '.'
```
...
- [Hanzo Documentation](https://github.com/hanzoai/s3/wiki)
