...
show_rdma_settings() {
    ...
    print_info "For optimal RDMA performance with Hanzo:"
    echo ""
...
generate_summary() {
    ...
    if [ "$ucx_ok" -eq 1 ] && [ "$devices_ok" -eq 1 ]; then
        print_info "🎉 UCX is ready for Hanzo RDMA integration!"
