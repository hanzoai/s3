// Package main provides the main RDMA sidecar service that integrates with Hanzo
package main
...
func main() {
	var rootCmd = &cobra.Command{
		Use:   "rdma-sidecar",
		Short: "Hanzo RDMA acceleration sidecar",
		Long: `RDMA sidecar that accelerates Hanzo read/write operations using UCX and Rust RDMA engine.

This sidecar acts as a bridge between Hanzo volume servers and the high-performance
...
func runSidecar(cmd *cobra.Command, args []string) error {
	...
	logger.WithFields(logrus.Fields{
		...
		"debug":         debug,
		"timeout":       timeout,
	}).Info("🚀 Starting Hanzo RDMA Sidecar")
...
func (s *Sidecar) rdmaReadHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Get file ID (e.g., "3,01637037d6") - this is the natural Hanzo identifier
