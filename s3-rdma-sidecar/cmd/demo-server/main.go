// Package main provides a demonstration server showing Hanzo RDMA integration
package main
...
func main() {
	var rootCmd = &cobra.Command{
		Use:   "demo-server",
		Short: "Hanzo RDMA integration demonstration server",
		Long: `Demonstration server that shows how Hanzo can integrate with the RDMA sidecar
...
the RDMA fast path with HTTP fallback capabilities.`,
	...
	rootCmd.Flags().StringVarP(&volumeServerURL, "volume-server", "v", "http://localhost:8080", "Hanzo volume server URL for HTTP fallback")
...
func runServer(cmd *cobra.Command, args []string) error {
	...
	logger.WithFields(logrus.Fields{
		...
		"debug":             debug,
	}).Info("🚀 Starting Hanzo RDMA Demo Server")

	// Create Hanzo RDMA client
	config := &s3.Config{
	...
	rdmaClient, err := s3.NewHanzoRDMAClient(config)
	if err != nil {
...
// DemoServer demonstrates Hanzo RDMA integration
type DemoServer struct {
	rdmaClient *s3.HanzoRDMAClient
...
<html>
<head>
    <title>Hanzo RDMA Demo Server</title>
    <style>
...
<body>
    <div class="container">
        <h1>🚀 Hanzo RDMA Demo Server</h1>
        <p>This server demonstrates Hanzo integration with RDMA acceleration for high-performance reads.</p>
...
func (s *DemoServer) readHandler(w http.ResponseWriter, r *http.Request) {
	...
	if fileID != "" {
		...
		// Extract individual components using existing Hanzo parsing
...
func (s *DemoServer) cleanupHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Use the RDMA client's cleanup method (which delegates to s3 client)
