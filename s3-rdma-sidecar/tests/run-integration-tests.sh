...
# Hanzo RDMA Integration Test Suite
...
test_hanzo_master() {
    ...
    if echo "$response" | jq -e '.IsLeader == true' > /dev/null; then
        ...
        print_error "Hanzo Master is not ready"
        echo "$response"
        return 1
    fi
}
...
test_hanzo_volume() {
    ...
    if echo "$response" | jq -e '.Version' > /dev/null; then
        ...
        print_error "Hanzo Volume Server is not ready"
        echo "$response"
        return 1
    fi
}
...
main() {
    ...
    echo -e "  ✅ Hanzo Master: Working"
    echo -e "  ✅ Hanzo Volume Server: Working"
    ...
    print_success "Hanzo RDMA integration is fully functional!"
