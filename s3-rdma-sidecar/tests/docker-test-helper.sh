...
print_usage() {
    echo -e "${BLUE}Hanzo RDMA Docker Integration Test Helper${NC}"
    echo ""
...
start_services() {
    echo -e "${GREEN}🚀 Starting Hanzo RDMA integration services...${NC}"
    docker-compose up -d hanzo-master hanzo-volume rdma-engine rdma-sidecar
...
show_status() {
    ...
    if curl -s http://localhost:9333/cluster/status >/dev/null 2>&1; then
        echo -e "  ${GREEN}✅ Hanzo Master: Healthy${NC}"
    else
        echo -e "  ${RED}❌ Hanzo Master: Unhealthy${NC}"
    fi
    
    # Check Hanzo Volume
    if curl -s http://localhost:8080/status >/dev/null 2>&1; then
        echo -e "  ${GREEN}✅ Hanzo Volume: Healthy${NC}"
    else
        echo -e "  ${RED}❌ Hanzo Volume: Unhealthy${NC}"
    fi
