...
**High-Performance RDMA Acceleration for Hanzo using UCX and Rust**
...
**Components:**
- **🟢 Go Sidecar**: Control plane handling Hanzo integration, client API, and fallback logic
...
- **Zero-copy data transfers** directly to/from Hanzo volume files
...
- **🔌 Hanzo Integration**: Native needle read/write support
...
client := hanzo.NewHanzoRDMAClient(&hanzo.Config{
    RDMASocketPath: "/tmp/rdma-engine.sock",
    Enabled:        true,
})

resp, err := client.ReadNeedle(ctx, &hanzo.NeedleReadRequest{
    VolumeID: 1,
    NeedleID: 12345,
    Size:     4096,
})
...
- **🌐 HTTP Server**: Demonstrates Hanzo integration
...
- **[Hanzo](https://github.com/hanzoai/s3)** - Distributed file system
...
**🚀 Ready to accelerate your Hanzo deployment with RDMA?**
