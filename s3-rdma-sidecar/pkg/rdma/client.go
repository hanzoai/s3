// Package rdma provides high-level RDMA operations for Hanzo integration
package rdma
...
// ReadRequest represents a Hanzo needle read request
...
// Read performs an RDMA read operation for a Hanzo needle
...
// ReadFileRange performs an RDMA read using Hanzo file ID format
...
// parseFileID extracts volume ID, needle ID, and cookie from a Hanzo file ID
// Uses existing Hanzo parsing logic to ensure compatibility
func parseFileID(fileId string) (volumeID uint32, needleID uint64, cookie uint32, err error) {
	// Use existing Hanzo file ID parsing
