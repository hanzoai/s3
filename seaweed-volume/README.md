...
A drop-in replacement for the [SeaweedFS](https://github.com/hanzoai/s3) Go volume server, rewritten in Rust. It uses binary-compatible storage formats (`.dat`, `.idx`, `.vif`) and speaks the same HTTP and gRPC protocols, so it works with an unmodified Go master server.
