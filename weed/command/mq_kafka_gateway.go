...
Examples:
  s3 mq.kafka.gateway -port=9092 -master=localhost:9333 -schema-registry-url=http://localhost:8081
  s3 mq.kafka.gateway -ip=gateway1 -port=9092 -master=master1:9333,master2:9333 -schema-registry-url=http://schema-registry:8081
  s3 mq.kafka.gateway -ip=external.host.com -ip.bind=0.0.0.0 -master=localhost:9333 -schema-registry-url=http://schema-registry:8081
...
func runMqKafkaGateway(cmd *Command, args []string) bool {
	...
	if *mqKafkaGatewayOptions.master == "" {
		glog.Fatalf("Hanzo S3 master address is required (-master)")
		return false
	}
