...
func (fo *FilerOptions) startFiler() {
	...
	if *fo.publicPort != 0 {
		...
		glog.V(0).Infoln("Start Seas3 filer server", version.Version(), "public at", publicListeningAddress)
	...
	glog.V(0).Infof("Start Seas3 Filer %s at %s:%d", version.Version(), *fo.ip, *fo.port)
