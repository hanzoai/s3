...
func (s3opt *S3Options) startS3Server() bool {
	...
	if *s3opt.tlsPrivateKey != "" {
		...
		if *s3opt.portHttps == 0 {
			glog.V(0).Infof("Start Seas3 S3 API Server %s at https port %d", version.Version(), *s3opt.port)
			...
			glog.V(0).Infof("Start Seas3 S3 API Server %s at https port %d", version.Version(), *s3opt.portHttps)
	...
	if *s3opt.tlsPrivateKey == "" || *s3opt.portHttps > 0 {
		glog.V(0).Infof("Start Seas3 S3 API Server %s at http port %d", version.Version(), *s3opt.port)
		if s3ApiLocalListener != nil {
			go func() {
