...
func startMaster(masterOption MasterOptions, masterWhiteList []string) {
	...
	glog.V(0).Infof("Start Seas3 Master %s at %s", version.Version(), listeningAddress)
	...
	glog.V(0).Infof("Start Seas3 Master %s grpc server at %s:%d", version.Version(), *masterOption.ipBind, grpcPort)
	if grpcLocalL != nil {
