...
func runWebDav(cmd *Command, args []string) bool {
	...
	glog.V(0).Infof("Starting Seas3 WebDav Server %s at %s", version.Version(), listenAddress)
...
func (wo *WebDavOption) startWebDav() bool {
	...
	if *wo.tlsPrivateKey != "" {
		glog.V(0).Infof("Start Seas3 WebDav Server %s at https %s", version.Version(), listenAddress)
		...
		glog.V(0).Infof("Start Seas3 WebDav Server %s at http %s", version.Version(), listenAddress)
