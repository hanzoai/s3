...
use hanzo_volume::storage::needle_map::NeedleMapKind;
use hanzo_volume::storage::store::Store;
use hanzo_volume::storage::types::{DiskType, Version, VolumeId};
...
fn test_state_with_guard(
    ...
    store
        .add_location(
            ...
            10,
            DiskType::HardDrive,
            hanzo_volume::config::MinFreeSpace::Percent(1.0),
            Vec::new(),
        )
    ...
    let state = Arc::new(VolumeServerState {
        ...
        read_mode: hanzo_volume::config::ReadMode::Local,
        ...
        metrics_runtime: std::sync::RwLock::new(
            hanzo_volume::server::volume_server::RuntimeMetricsConfig::default(),
        ),
...
async fn admin_router_serves_volume_ui_static_assets() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri("/hanzostatic/bootstrap/3.3.1/css/bootstrap.min.css")
                .body(Body::empty())
                .unwrap(),
        )
        .await
