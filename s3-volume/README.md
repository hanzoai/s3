# Hanzo Volume Server (Rust)

A drop-in replacement for the [Hanzo](https://github.com/hanzoai/s3) Go volume server, rewritten in Rust. It uses binary-compatible storage formats (`.dat`, `.idx`, `.vif`) and speaks the same HTTP and gRPC protocols, so it works with an unmodified Go master server.
...
The binary is produced at `target/release/hanzo-volume`.
...
```bash
# Minimal
hanzo-volume --port 8080 --master localhost:9333 --dir /data/vol1 --max 7

# Multiple data directories
hanzo-volume --port 8080 --master localhost:9333 \
...
hanzo-volume --port 8080 --master localhost:9333 --dir /data/vol1 --max 7 \
  --securityFile /etc/hanzo/security.toml
...
- **gRPC** (Tonic 0.12) -- all `VolumeServer` RPCs from the Hanzo protobuf definition.
