...
/// Hanzo Volume Server (Rust implementation)
///
/// Start a volume server to provide storage spaces.
#[derive(Parser, Debug)]
#[command(name = "s3-volume", version, about)]
pub struct Cli {
    /// HTTP listen port
...
fn default_security_config_candidates(
    ...
    if let Some(home) = home_dir {
        candidates.push(home.join(".s3").join(SECURITY_CONFIG_FILE_NAME));
    }
    candidates.push(PathBuf::from("/usr/local/etc/hanzo").join(SECURITY_CONFIG_FILE_NAME));
    candidates.push(PathBuf::from("/etc/hanzo").join(SECURITY_CONFIG_FILE_NAME));
    candidates
}
...
"#,
    ...
    fn test_parse_security_config_discovers_home_default() {
        ...
        let hanzo_home = home_dir.path().join(".s3");
        std::fs::create_dir_all(&hanzo_home).unwrap();
        std::fs::write(
            hanzo_home.join(SECURITY_CONFIG_FILE_NAME),
            r#"
[jwt.signing]
key = "home-secret"
"#,
    ...
    fn test_parse_security_config_uses_grpc_root_ca() {
        ...
        std::fs::write(
            ...
            r#"
[grpc]
ca = "/etc/hanzo/grpc-ca.pem"

[grpc.volume]
cert = "/etc/hanzo/volume-cert.pem"
key = "/etc/hanzo/volume-key.pem"
"#,
        )
        .unwrap();
        ...
        with_cleared_security_env(|| {
            let cfg = parse_security_config(tmp.path().to_str().unwrap());
            assert_eq!(cfg.grpc_ca_file, "/etc/hanzo/grpc-ca.pem");
            assert_eq!(cfg.grpc_cert_file, "/etc/hanzo/volume-cert.pem");
            assert_eq!(cfg.grpc_key_file, "/etc/hanzo/volume-key.pem");
        });
    }
...
[https.client]
enabled = true
cert = "/etc/hanzo/client-cert.pem"
key = "/etc/hanzo/client-key.pem"
ca = "/etc/hanzo/client-ca.pem"
"#,
        )
        .unwrap();
        ...
        with_cleared_security_env(|| {
            ...
            assert_eq!(cfg.https_client_cert_file, "/etc/hanzo/client-cert.pem");
            assert_eq!(cfg.https_client_key_file, "/etc/hanzo/client-key.pem");
            assert_eq!(cfg.https_client_ca_file, "/etc/hanzo/client-ca.pem");
        });
    }
