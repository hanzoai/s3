...
async fn lookup_volume(
    ...
    // master_url may be in Hanzo's "host:httpPort.grpcPort" form (mirrors
...
pub async fn post_handler(
    ...
    // Extract Hanzo-* custom metadata headers (pairs)
    // Go's net/http canonicalizes header names to Title-Case, so after stripping
    // the "Hanzo-" prefix, keys are Title-Case (e.g., "Foo-Bar"). Rust's http
    ...
    let pair_map: std::collections::HashMap<String, String> = headers
        ...
        .filter_map(|(k, v)| {
            let key = k.as_str();
            if key.len() > 8 && key[..8].eq_ignore_ascii_case("hanzo-") {
...
mod tests {
    ...
    ///
    /// Hanzo encodes a server's gRPC port by appending `.grpcPort` to
