...
use crate::pb::master_pb::hanzo_client::HanzoClient;
...
impl VolumeGrpcService {
    ...
    async fn notify_master_volume_readonly(
        ...
        let mut client = HanzoClient::with_interceptor(
            channel,
...
/// Build a gRPC endpoint from a Hanzo server address.
fn to_grpc_endpoint(
    target: &str,
...
async fn ping_master_target(
    ...
    let mut client = master_pb::hanzo_client::HanzoClient::with_interceptor(
        channel,
        super::request_id::outgoing_request_id_interceptor,
    )
    .max_decoding_message_size(GRPC_MAX_MESSAGE_SIZE)
...
async fn ping_filer_target(
    ...
    let mut client = filer_pb::hanzo_filer_client::HanzoFilerClient::with_interceptor(
        channel,
        super::request_id::outgoing_request_id_interceptor,
    )
    .max_decoding_message_size(GRPC_MAX_MESSAGE_SIZE)
...
mod tests {
    ...
    fn test_parse_grpc_address_with_explicit_grpc_port() {
        // Format: "ip:port.grpcPort" — used by Hanzo for source_data_node
