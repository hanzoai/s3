//! Distributed EC read path. Mirror of `s3/storage/store_ec.go`'s
...
use crate::pb::master_pb::{self, hanzo_client::HanzoClient, LookupEcVolumeRequest};
...
async fn cached_lookup_ec_shard_locations(
    ...
    let mut client = HanzoClient::with_interceptor(channel, outgoing_request_id_interceptor)
...
/// Build a Hanzo-style `host:httpPort.grpcPort` address from a
