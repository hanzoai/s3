...
impl DiskLocation {
    ...
    /// the orphan-shard layout reported in hanzo/hanzo#9212.
    ///
    /// Mirrors `DiskLocation.HasEcxFileOnDisk` in
    /// `s3/storage/disk_location_ec.go`. Skips entries that are
    ...
    /// `s3/storage/disk_location_ec.go`): when a volume's shards live
    /// on this disk but the index files were left on a sibling disk
    /// (the hanzo/hanzo#9212 orphan-shard layout), the
    ...
    ///
    /// Mirrors `loadAllEcShards` in `s3/storage/disk_location_ec.go`.
    ...
    /// VolumeEcShardsMount — strict superset of hanzo/hanzo#9212.
    ...
    /// `s3/storage/disk_location_ec.go`.
...
/// `s3/storage/disk_location.go`. Used when iterating raw filenames
