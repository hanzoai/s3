...
use hanzo_volume::config::{self, VolumeServerConfig};
use hanzo_volume::metrics;
use hanzo_volume::pb::volume_server_pb::volume_server_server::VolumeServerServer;
use hanzo_volume::security::tls::{
    build_rustls_server_config, build_rustls_server_config_with_grpc_client_auth,
    GrpcClientAuthPolicy, TlsPolicy,
};
use hanzo_volume::security::{Guard, SigningKey};
#[cfg(unix)]
use hanzo_volume::server::debug::build_debug_router;
use hanzo_volume::server::grpc_client::load_outgoing_grpc_tls;
use hanzo_volume::server::grpc_server::VolumeGrpcService;
#[cfg(unix)]
use hanzo_volume::server::profiling::CpuProfileSession;
use hanzo_volume::server::request_id::GrpcRequestIdLayer;
use hanzo_volume::server::volume_server::{
    build_metrics_router, RuntimeMetricsConfig, VolumeServerState,
};
use hanzo_volume::server::write_queue::WriteQueue;
use hanzo_volume::storage::store::Store;
use hanzo_volume::storage::types::DiskType;
...
fn main() {
    ...
    hanzo_volume::server::server_stats::init_process_start();
    #[cfg(unix)]
    ...
    info!(
        "Hanzo Volume Server (Rust) v{}",
        hanzo_volume::version::full_version()
    );
...
async fn run(
    ...
    let state = Arc::new(VolumeServerState {
        ...
        s3_tier_registry: std::sync::RwLock::new(
            hanzo_volume::remote_storage::s3_tier::S3TierRegistry::new(),
        ),
    ...
    if let Some(saved) =
        hanzo_volume::server::grpc_server::load_state_file(&state.state_file_path)
    {
        state
            .maintenance
    ...
    if !config.masters.is_empty() {
        let hb_config = hanzo_volume::server::heartbeat::HeartbeatConfig {
            ip: config.ip.clone(),
            port: config.port,
        ...
        hanzo_volume::server::heartbeat::prime_master_configuration(&hb_config, &state).await;
    }
    ...
    // Build HTTP routers
    #[allow(unused_mut)]
    let mut admin_router = hanzo_volume::server::volume_server::build_admin_router_with_ui(
        state.clone(),
        config.ui_enabled,
    );
    #[cfg(unix)]
    ...
    let grpc_handle = {
        ...
        tokio::spawn(async move {
            ...
            let reflection_v1 = tonic_reflection::server::Builder::configure()
                .register_encoded_file_descriptor_set(hanzo_volume::pb::FILE_DESCRIPTOR_SET)
                .build_v1()
    ...
    let heartbeat_handle = {
        ...
        if !master_addrs.is_empty() {
            ...
            Some(tokio::spawn(async move {
                hanzo_volume::server::heartbeat::run_heartbeat_with_state(
                    hb_config,
                    hb_state,
                    hb_shutdown,
                )
    ...
    let public_handle = if needs_public {
        let public_router =
            hanzo_volume::server::volume_server::build_public_router(state.clone());
...
mod tests {
    ...
    use hanzo_volume::security::tls::TlsPolicy;
    ...
    fn test_effective_http_tls_policy_ignores_tls_policy_without_ca() {
        ...
        assert_eq!(
            effective_http_tls_policy("/etc/hanzo/http-ca.pem", &configured),
            configured
        );
    }
