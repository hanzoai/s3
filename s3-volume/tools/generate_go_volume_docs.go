...
func main() {
	...
	outFlag := flag.String("out", "hanzo-volume/docs/go-volume-server", "output directory for generated markdown")
	flag.Parse()
...
func shouldInclude(rel string) bool {
	switch {
	case rel == "s3/command/volume.go":
		return true
	case rel == "s3/server/common.go":
		return true
	case rel == "s3/server/constants/volume.go":
		return true
	case rel == "s3/server/volume_server_ui/templates.go":
		return true
	case strings.HasPrefix(rel, "s3/server/volume") && strings.HasSuffix(rel, ".go"):
		return true
	case strings.HasPrefix(rel, "s3/storage/"):
		return true
	case strings.HasPrefix(rel, "s3/images/"):
		return true
	case strings.HasPrefix(rel, "s3/security/"):
		return true
	case strings.HasPrefix(rel, "s3/stats/"):
		return true
	default:
		return false
	}
...
func deriveEffect(info *FunctionInfo, decl *ast.FuncDecl) string {
	...
	case strings.Contains(name, "Needle"):
		return "Manipulates or transports Hanzo needle state."
	default:
...
func renderIndexReadme(outRel string, docs []*FileDoc) string {
	...
	b.WriteString("Generated reference set for translating the Go Hanzo volume server into the Rust `hanzo-volume` crate.\n\n")
	b.WriteString("- Generated at: `" + time.Now().Format(time.RFC3339) + "`\n")
	b.WriteString(fmt.Sprintf("- Markdown files: `%d`\n", len(docs)))
	b.WriteString("- Scope: `s3/command/volume.go`, selected `s3/server` volume-server files, and runtime files under `s3/storage`, `s3/images`, `s3/security`, and `s3/stats`.\n")
...
func rustCounterparts(rel string) []string {
	switch {
	case rel == "s3/command/volume.go":
		return []string{"hanzo-volume/src/config.rs", "hanzo-volume/src/main.rs"}
	case strings.HasPrefix(rel, "s3/images/"):
		return []string{"hanzo-volume/src/images.rs"}
	case strings.HasPrefix(rel, "s3/security/"):
		return []string{"hanzo-volume/src/security.rs"}
	case strings.HasPrefix(rel, "s3/stats/"):
		return []string{"hanzo-volume/src/metrics.rs"}
	case rel == "s3/server/common.go":
		return []string{"hanzo-volume/src/server/handlers.rs", "hanzo-volume/src/server/volume_server.rs", "hanzo-volume/src/main.rs"}
	case rel == "s3/server/constants/volume.go":
		return []string{"hanzo-volume/src/server/mod.rs", "hanzo-volume/src/server/volume_server.rs"}
	case rel == "s3/server/volume_server.go":
		return []string{"hanzo-volume/src/server/volume_server.rs", "hanzo-volume/src/server/heartbeat.rs", "hanzo-volume/src/main.rs"}
	case strings.HasPrefix(rel, "s3/server/volume_server_handlers"):
		return []string{"hanzo-volume/src/server/handlers.rs", "hanzo-volume/src/server/volume_server.rs"}
	case strings.HasPrefix(rel, "s3/server/volume_grpc_"):
		return []string{"hanzo-volume/src/server/grpc_server.rs", "hanzo-volume/src/server/heartbeat.rs"}
	case rel == "s3/server/volume_server_ui/templates.go":
		return []string{"hanzo-volume/src/server/volume_server.rs"}
	case rel == "s3/storage/disk_location.go" || rel == "s3/storage/disk_location_ec.go":
		return []string{"hanzo-volume/src/storage/disk_location.rs"}
	case strings.HasPrefix(rel, "s3/storage/erasure_coding/"):
		name := filepath.Base(rel)
		switch name {
		case "ec_decoder.go":
			return []string{"hanzo-volume/src/storage/erasure_coding/ec_decoder.rs"}
		case "ec_encoder.go":
			return []string{"hanzo-volume/src/storage/erasure_coding/ec_encoder.rs"}
		case "ec_locate.go":
			return []string{"hanzo-volume/src/storage/erasure_coding/ec_locate.rs"}
		case "ec_shard.go", "ec_shard_info.go", "ec_shards_info.go":
			return []string{"hanzo-volume/src/storage/erasure_coding/ec_shard.rs"}
		default:
			return []string{"hanzo-volume/src/storage/erasure_coding/ec_volume.rs", "hanzo-volume/src/storage/erasure_coding/mod.rs"}
		}
	case strings.HasPrefix(rel, "s3/storage/needle/"):
		name := filepath.Base(rel)
		switch name {
		case "crc.go":
			return []string{"hanzo-volume/src/storage/needle/crc.rs"}
		case "volume_ttl.go":
			return []string{"hanzo-volume/src/storage/needle/ttl.rs"}
		default:
			return []string{"hanzo-volume/src/storage/needle/needle.rs", "hanzo-volume/src/storage/needle/mod.rs"}
		}
	case rel == "s3/storage/needle_map.go" || strings.HasPrefix(rel, "s3/storage/needle_map/") || strings.HasPrefix(rel, "s3/storage/needle_map_"):
		name := filepath.Base(rel)
		if strings.Contains(name, "compact_map") {
			return []string{"hanzo-volume/src/storage/needle_map/compact_map.rs"}
		}
		return []string{"hanzo-volume/src/storage/needle_map.rs"}
	case strings.HasPrefix(rel, "s3/storage/store"):
		return []string{"hanzo-volume/src/storage/store.rs"}
	case strings.HasPrefix(rel, "s3/storage/super_block/"):
		return []string{"hanzo-volume/src/storage/super_block.rs"}
	case strings.HasPrefix(rel, "s3/storage/types/"):
		return []string{"hanzo-volume/src/storage/types.rs"}
	case strings.HasPrefix(rel, "s3/storage/backend/s3_backend/"):
		return []string{"hanzo-volume/src/remote_storage/s3.rs", "hanzo-volume/src/remote_storage/s3_tier.rs"}
	case strings.HasPrefix(rel, "s3/storage/backend/"):
		return []string{"hanzo-volume/src/storage/volume.rs", "hanzo-volume/src/storage/mod.rs"}
	case strings.HasPrefix(rel, "s3/storage/idx/"):
		return []string{"hanzo-volume/src/storage/idx/mod.rs"}
	case strings.HasPrefix(rel, "s3/storage/volume"):
		return []string{"hanzo-volume/src/storage/volume.rs"}
	case strings.HasPrefix(rel, "s3/storage/"):
		return []string{"hanzo-volume/src/storage/mod.rs"}
	default:
		return nil
	}
}
