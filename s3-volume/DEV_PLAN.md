...
- **Production Sprint 5** — Upload Compatibility:
  ...
  - Hanzo-* custom metadata headers stored as needle pairs (JSON, max 64KB)
...
```bash
# Build
cd hanzo-volume && cargo build --release
...
# Run Rust unit tests
cd hanzo-volume && cargo test

# Test heartbeat with Go master
s3 master -port=9333 &
hanzo-volume --port 8080 --master localhost:9333 --dir /tmp/vol1 --max 7
