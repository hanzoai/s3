...
//
// Each adapter embeds grpc.ServerStream (nil) for the metadata/SendMsg/RecvMsg
// methods the engine never calls on these paths, and serves Context() from the
// per-stream s.Context() (zap-proto/go v1.6.2+): cancelled when the stream ends
// OR the peer drops the connection, so a long-lived idle subscription that then
...
// --- grpc.ServerStream stub shared by every server adapter -------------------
//
// serverStreamStub satisfies the metadata/SendMsg/RecvMsg half of
// grpc.ServerStream that the engine methods never call on these paths. Each
// concrete adapter overrides Context() with the per-stream context; the
// remaining methods are inert.
type serverStreamStub struct{ grpc.ServerStream }
