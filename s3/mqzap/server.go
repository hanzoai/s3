...
// to bytes. With NewServerBackend wired into mq_brokerwire.Dispatch (the analogue
// of the gRPC RegisterHanzoMessagingServer) and NewStreamServer into the stream
// handler, the whole MQ broker (14 unary + 7 streaming RPCs) answers over ZAP —
// no protobuf framing on the wire, all engine logic reused unchanged.
//
