...
import (
	...
	"github.com/hanzoai/s3/s3/iam/ldap"
...
func testJWTAuthorizationWithRole(t *testing.T, iam *IdentityAccessManagement, identity *Identity, action Action, bucket, object, token, roleName string) bool {
	...
	req.Header.Set("Authorization", "Bearer "+token)
	req.Header.Set(s3_constants.HanzoSessionTokenHeader, token)
	...
	req.Header.Set(s3_constants.HanzoPrincipalHeader, principalArn)
