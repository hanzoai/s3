...
func (s3a *S3ApiServer) putToFiler(r *http.Request, filePath string, dataReader io.Reader, bucket string, object string, partNumber int, lifecycleTTLSec int32, finalize *putFinalize, uniqueWritePath bool) (etag string, code s3err.ErrorCode, sseMetadata SSEResponseMetadata) {
	...
	if customerKey != nil && len(sseIV) > 0 {
		...
		entry.Extended[s3_constants.HanzoSSEIV] = sseIV
	...
	if sseKMSKey != nil {
		...
		entry.Extended[s3_constants.HanzoSSEKMSKey] = sseKMSMetadata
	...
	if sseS3Key != nil && len(sseS3Metadata) > 0 {
		// Store metadata as RAW bytes (matches filer behavior - filer decodes base64 headers and stores raw bytes)
		entry.Extended[s3_constants.HanzoSSES3Key] = sseS3Metadata
	...
	createUnderLock := func() s3err.ErrorCode {
		createErr = s3a.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
...
func filerErrorToS3Error(err error) s3err.ErrorCode {
	...
	case errors.Is(err, s3server.ErrReadOnly):
...
// starting with "Hanzo-" in entry.Extended during file creation, ensuring atomic metadata persistence.
//
...
func (s3a *S3ApiServer) putSuspendedVersioningObject(r *http.Request, bucket, object string, dataReader io.Reader, objectContentType string) (etag string, errCode s3err.ErrorCode, sseMetadata SSEResponseMetadata) {
	...
	// The filer automatically stores any header starting with "Hanzo-" in entry.Extended
