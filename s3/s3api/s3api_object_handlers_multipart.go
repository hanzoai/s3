...
func (s3a *S3ApiServer) PutObjectPartHandler(w http.ResponseWriter, r *http.Request) {
	...
	if sseCustomerAlgorithm == "" && uploadEntry.Extended != nil {
		if keyIDBytes, exists := uploadEntry.Extended[s3_constants.HanzoSSEKMSKeyID]; exists {
			keyID := string(keyIDBytes)

			bucketKeyEnabled := false
			if bucketKeyBytes, exists := uploadEntry.Extended[s3_constants.HanzoSSEKMSBucketKeyEnabled]; exists && string(bucketKeyBytes) == "true" {
				bucketKeyEnabled = true
			}

			var encryptionContext map[string]string
			if contextBytes, exists := uploadEntry.Extended[s3_constants.HanzoSSEKMSEncryptionContext]; exists {
			...
			var baseIV []byte
			if baseIVBytes, exists := uploadEntry.Extended[s3_constants.HanzoSSEKMSBaseIV]; exists {
			...
			r.Header.Set(s3_constants.HanzoSSEKMSBaseIVHeader, base64.StdEncoding.EncodeToString(baseIV))
		} else {
...
func (s3a *S3ApiServer) getMultipartSSEAlgorithm(bucket, uploadID string) string {
	...
	if _, ok := entry.Extended[s3_constants.HanzoSSEKMSKeyID]; ok {
		return "aws:kms"
	}
	if _, ok := entry.Extended[s3_constants.HanzoSSES3Encryption]; ok {
		return "AES256"
	}
	return ""
}
...
func (s3a *S3ApiServer) handleSSES3MultipartHeaders(r *http.Request, uploadEntry *filer_pb.Entry, uploadID string) error {
	if encryptionTypeBytes, exists := uploadEntry.Extended[s3_constants.HanzoSSES3Encryption]; exists && string(encryptionTypeBytes) == s3_constants.SSEAlgorithmAES256 {
		...
		var baseIV []byte
		if baseIVBytes, exists := uploadEntry.Extended[s3_constants.HanzoSSES3BaseIV]; exists {
		...
		if keyDataBytes, exists := uploadEntry.Extended[s3_constants.HanzoSSES3KeyData]; exists {
			// Key data is already base64 encoded, pass it directly
			keyDataStr := string(keyDataBytes)
			r.Header.Set(s3_constants.HanzoSSES3KeyDataHeader, keyDataStr)
		...
		r.Header.Set(s3_constants.HanzoSSES3BaseIVHeader, base64.StdEncoding.EncodeToString(baseIV))
