...
func TestAuthorizeWithIAMSessionTokenExtraction(t *testing.T) {
	t.Run("Extracts X-Hanzo-Session-Token from JWT auth", func(t *testing.T) {
		req := &http.Request{
			Header: http.Header{
				"X-Hanzo-Session-Token": {"jwt-token-123"},
				"X-Hanzo-Principal":     {"arn:aws:iam::user/test"},
			},
			URL: &url.URL{},
		}

		// Extract tokens the same way authorizeWithIAM does
		sessionToken := req.Header.Get(s3_constants.HanzoSessionTokenHeader)
		principal := req.Header.Get(s3_constants.HanzoPrincipalHeader)
