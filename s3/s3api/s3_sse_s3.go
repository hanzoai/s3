...
var legacyKEKWrappingSalt = []byte("hanzo-sse-s3-kek-wrapping-v1")
...
func deriveKeyFromSecret(secret string) ([]byte, error) {
	hkdfReader := hkdf.New(sha256.New, []byte(secret), nil, []byte("hanzo-sse-s3-kek"))
	key := make([]byte, SSES3KeySize)
...
func (km *SSES3KeyManager) updateKEKContent(content []byte) error {
	...
	return km.filerClient.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
...
func (km *SSES3KeyManager) GetMasterKey() []byte {
	...
	// We use the KEK as the secret, and "hanzo-sts-signing-key" as the info
	hkdfReader := hkdf.New(sha256.New, km.superKey, nil, []byte("hanzo-sts-signing-key"))
...
func (k *KeyManagerFilerClient) WithFilerClient(streamingMode bool, fn func(filer_pb.HanzoFilerClient) error) error {
...
func InitializeGlobalSSES3KeyManager(filerClient *wdclient.FilerClient, grpcDialOption pb.DialOption) error {
