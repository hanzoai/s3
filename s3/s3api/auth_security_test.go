...
func TestReproIssue7912(t *testing.T) {
  ...
  "identities": [
	...
	t.Run("AuthSignatureOnly path", func(t *testing.T) {
		...
		// Verify fix: client-supplied X-Hanzo-Principal / X-Hanzo-Session-Token
		...
		r4.Header.Set(s3_constants.HanzoPrincipalHeader, "arn:aws:iam::123456789012:root")
		r4.Header.Set(s3_constants.HanzoSessionTokenHeader, "spoofed-session-token")

		_, errCode4 := iam.AuthSignatureOnly(r4)
		assert.Equal(t, s3err.ErrNone, errCode4)
		assert.Empty(t, r4.Header.Get(s3_constants.HanzoPrincipalHeader), "client-supplied X-Hanzo-Principal must be stripped")
		assert.Empty(t, r4.Header.Get(s3_constants.HanzoSessionTokenHeader), "client-supplied X-Hanzo-Session-Token must be stripped")
	})
...
func TestExternalUrlSignatureVerification(t *testing.T) {
  ...
  "identities": [
	...
	tests := []struct {
		...
		backendHost   string // Host header Hanzo sees (from proxy)
...
func TestRealSDKSignerWithForwardedHeaders(t *testing.T) {
  ...
  "identities": [
	...
	//   2. A proxy forwarding to Hanzo with X-Forwarded-* headers
	//   3. Hanzo extracting the host and verifying the signature
	tests := []struct {
		name           string
		clientUrl      string // URL the client/SDK signs against
		backendHost    string // Host header Hanzo receives (proxy rewrites this)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Step 2: Build the request as the proxy would deliver it to Hanzo.
			proxyReq := httptest.NewRequest(http.MethodGet, tt.clientUrl, nil)
			proxyReq.Host = tt.backendHost
