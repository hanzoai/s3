# Hanzo Policy Evaluation Engine

This document describes the comprehensive policy evaluation engine that has been added to Hanzo, providing AWS S3-compatible policy support while maintaining full backward compatibility with existing `identities.json` configuration.
...
The policy engine provides a seamless upgrade path from Hanzo's existing simple IAM system to full AWS S3-compatible policies, giving you the best of both worlds: simplicity for basic use cases and power for complex enterprise scenarios.
...
For feature requests or to track progress, see the [GitHub Issues](https://github.com/hanzoai/s3/issues). 