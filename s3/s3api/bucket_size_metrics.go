...
func (s3a *S3ApiServer) enforceBucketQuotas(ctx context.Context, buckets []*filer_pb.Entry, collectionInfos map[string]*CollectionInfo) {
	...
	if err := s3a.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
		return filer.SaveInsideFiler(ctx, client, filer.DirectoryEtcHanzo, filer.FilerConfName, buf.Bytes())
	}); err != nil {
...
func (s3a *S3ApiServer) collectCollectionInfoFromMaster(ctx context.Context) (map[string]*CollectionInfo, error) {
	...
	err := pb.WithOneOfGrpcMasterClients(false, masterMap, s3a.option.GrpcDialOption, func(client master_pb.HanzoClient) error {
