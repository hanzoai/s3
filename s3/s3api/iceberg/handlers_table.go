...
func (s *Server) handleListTables(w http.ResponseWriter, r *http.Request) {
	...
	err = s.filerClient.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
...
func (s *Server) handleCreateTable(w http.ResponseWriter, r *http.Request) {
	...
	existsErr := s.filerClient.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
		mgrClient := s3tables.NewManagerClient(client)
	...
	if err != nil {
		if tableErr, ok := err.(*s3tables.S3TablesError); ok && tableErr.Type == s3tables.ErrCodeTableAlreadyExists {
			...
			getErr := s.filerClient.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
				mgrClient := s3tables.NewManagerClient(client)
...
func (s *Server) handleDropTable(w http.ResponseWriter, r *http.Request) {
	...
	lookupErr := s.filerClient.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
		mgrClient := s3tables.NewManagerClient(client)
...
func (s *Server) cleanupStaleTableLocation(ctx context.Context, bucketName, tablePath string) error {
	...
	return s.filerClient.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
