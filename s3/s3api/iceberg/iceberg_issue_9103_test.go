...
func TestBuildFileIOConfig(t *testing.T) {
	...
	t.Run("endpoint is advertised with path-style-access and region", func(t *testing.T) {
		s := &Server{s3Endpoint: "http://hanzo.example:8333"}
		got := s.buildFileIOConfig()
		if got["s3.endpoint"] != "http://hanzo.example:8333" {
			t.Fatalf("s3.endpoint = %q, want %q", got["s3.endpoint"], "http://hanzo.example:8333")
		}
