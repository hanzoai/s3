...
func uploadEntryHasSSE(uploadEntry *filer_pb.Entry) bool {
	...
	if _, ok := uploadEntry.Extended[s3_constants.HanzoSSEKMSKeyID]; ok {
		return true
	}
	if v, ok := uploadEntry.Extended[s3_constants.HanzoSSES3Encryption]; ok && string(v) == s3_constants.SSEAlgorithmAES256 {
		return true
	}
	return false
}
...
func sourceEntryHasSSE(srcEntry *filer_pb.Entry) bool {
	...
	if srcEntry.Extended != nil {
		if _, ok := srcEntry.Extended[s3_constants.HanzoSSES3Key]; ok {
			return true
		}
		if _, ok := srcEntry.Extended[s3_constants.HanzoSSEKMSKey]; ok {
			return true
		}
...
func (s3a *S3ApiServer) openSSES3SourcePlaintextReader(
	...
	// Legacy single-part fallback: entry-level HanzoSSES3Key + entry IV.
	keyData, ok := srcEntry.Extended[s3_constants.HanzoSSES3Key]
	if !ok || len(keyData) == 0 {
...
func (s3a *S3ApiServer) applyDestSSEHeadersToCopyRequest(
	...
	if keyIDBytes, hasKMS := uploadEntry.Extended[s3_constants.HanzoSSEKMSKeyID]; hasKMS {
		...
		if v, ok := uploadEntry.Extended[s3_constants.HanzoSSEKMSBucketKeyEnabled]; ok && string(v) == "true" {
			bucketKeyEnabled = true
		}

		var encryptionContext map[string]string
		if cb, ok := uploadEntry.Extended[s3_constants.HanzoSSEKMSEncryptionContext]; ok {
		...
		var baseIV []byte
		if ivBytes, ok := uploadEntry.Extended[s3_constants.HanzoSSEKMSBaseIV]; ok {
