...
// the request through.
//
// Regression test for https://github.com/hanzoai/s3/issues/9072
...
func TestPresignedSignatureV4WithForwardedPrefix(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// 2. Proxy strips prefix and forwards to Hanzo with X-Forwarded-Prefix header
...
func TestIAMSignatureServiceMatching(t *testing.T) {
	...
	// Now test that Hanzo computes the same signature with our fix
