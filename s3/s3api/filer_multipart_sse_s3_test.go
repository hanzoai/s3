...
func TestCompletedMultipartChunkBackfillsSSES3MetadataFromUploadEntry(t *testing.T) {
	...
	uploadEntry := &filer_pb.Entry{
		Extended: map[string][]byte{
			s3_constants.HanzoSSES3Encryption: []byte(s3_constants.SSEAlgorithmAES256),
			s3_constants.HanzoSSES3BaseIV:     []byte(base64.StdEncoding.EncodeToString(baseIV)),
			s3_constants.HanzoSSES3KeyData:    []byte(base64.StdEncoding.EncodeToString(keyData)),
		},
	}
...
// SSE-S3 attributes onto the completed entry: HanzoSSES3Key (used by
...
func TestApplyMultipartSSES3HeadersFromUploadEntry(t *testing.T) {
	...
	t.Run("backfills missing canonical attributes", func(t *testing.T) {
		...
		if !bytes.Equal(finalEntry.Extended[s3_constants.HanzoSSES3Key], keyData) {
	...
	t.Run("does not clobber existing canonical attributes", func(t *testing.T) {
		...
		finalEntry := &filer_pb.Entry{Extended: map[string][]byte{
			s3_constants.HanzoSSES3Key:      existingKey,
			s3_constants.AmzServerSideEncryption: existingHeader,
		}}
		applyMultipartSSES3HeadersFromUploadEntry(finalEntry, sses3Info)
		if !bytes.Equal(finalEntry.Extended[s3_constants.HanzoSSES3Key], existingKey) {
