...
func LoadCompileInputs(ctx context.Context, client filer_pb.HanzoFilerClient, bucketsPath string) ([]engine.CompileInput, []ParseError, error) {
	var (
	...
	for {
		pageCount := 0
		var lastName string
		err := filer_pb.HanzoList(ctx, client, bucketsPath, "", func(entry *filer_pb.Entry, isLast bool) error {
			pageCount++
			lastName = entry.Name
