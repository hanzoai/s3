...
// filer caps HanzoList(..., limit=0) at DirListingLimit (1000 by
...
func FilerListFunc(client filer_pb.HanzoFilerClient, bucketsPath string) bootstrap.ListFunc {
...
func walkBucketTree(ctx context.Context, client filer_pb.HanzoFilerClient, dir, bucketRoot, start string, cb func(*bootstrap.Entry) error) error {
...
func expandVersionsDir(ctx context.Context, client filer_pb.HanzoFilerClient, bucketRoot, versionsKey string, versionsEntry *filer_pb.Entry, start string, skipBare map[string]bool, cb func(*bootstrap.Entry) error) error {
...
func lookupNullVersion(ctx context.Context, client filer_pb.HanzoFilerClient, bucketRoot, logical string) (*filer_pb.Entry, string, bool, bool) {
...
// listAll issues paginated HanzoList calls until exhausted. Ported
// from scheduler/bootstrap.go's same-named helper; Phase 5 deletes
// the scheduler copy when the streaming path is removed.
func listAll(ctx context.Context, client filer_pb.HanzoFilerClient, dir string, fn func(*filer_pb.Entry) error) error {
	pageSize := listPageSize.Load()
	startFrom := ""
	for {
		var pageCount uint32
		var lastName string
		if err := filer_pb.HanzoList(ctx, client, dir, "", func(e *filer_pb.Entry, _ bool) error {
			pageCount++
			if e != nil {
				lastName = e.Name
			}
