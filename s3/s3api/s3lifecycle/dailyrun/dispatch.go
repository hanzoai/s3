...
func buildDeleteRequest(m router.Match) []byte {
	rh := m.Key.RuleHash
	return s3_lifecyclewire.NewLifecycleDeleteRequest(s3_lifecyclewire.LifecycleDeleteRequestInput{
		...
		RuleHash:         rh[:],
		ActionKind:       toWireActionKind(m.Key.ActionKind),
		ExpectedIdentity: toWireIdentity(m.Identity),
	})
}

func toWireActionKind(k s3lifecycle.ActionKind) uint32 {
	switch k {
...
func toWireIdentity(id *router.EntryIdentity) *s3_lifecyclewire.EntryIdentityInput {
	if id == nil {
		return nil
	}
