...
// import cycle. req is a ZAP-encoded LifecycleDeleteRequest
// (s3_lifecyclewire.NewLifecycleDeleteRequest); the returned view exposes the
// server's Outcome()/Reason() over the ZAP transport.
type LifecycleClient interface {
	LifecycleDelete(ctx context.Context, req []byte) (s3_lifecyclewire.LifecycleDeleteResponse, error)
}
...
func processMatches(ctx context.Context, cfg Config, runNow time.Time, ev *reader.Event, matches []router.Match) (skippedAny, halted bool, err error) {
	for _, m := range matches {
		...
		stats.S3LifecycleDispatchCounter.WithLabelValues(m.Bucket, m.Key.ActionKind.String(), s3lifecycle.OutcomeString(outcome)).Inc()
		switch outcome {
		...
		case s3_lifecyclewire.LifecycleDeleteOutcomeRetryLater,
			...
			glog.V(1).Infof("daily_run: %s on %s/%s %s",
				s3lifecycle.OutcomeString(outcome), m.Bucket, m.ObjectKey, m.Key.ActionKind)
			return skippedAny, true, nil
		default:
