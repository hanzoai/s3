...
This document is the as-built reference. For operator-facing guides, see the [Hanzo wiki](https://github.com/hanzoai/s3/wiki).
...
Table uses the S3-spec rule names (what operators type in lifecycle XML). The corresponding engine constants in `s3/s3api/s3lifecycle/action_kind.go` are `ActionKindExpirationDays`, `ActionKindNoncurrentDays`, `ActionKindAbortMPU`, `ActionKindExpirationDate`, `ActionKindExpiredDeleteMarker`, `ActionKindNewerNoncurrent` — same one-to-one mapping, shorter spelling.
...
Persisted per shard at `/etc/s3/lifecycle/daily-cursors/shard-NN.json`. Shape (`s3/s3api/s3lifecycle/dailyrun/cursor.go`):
...
Per-shard Prometheus gauges (`s3/stats/metrics.go`):
...
Admin config (`s3/worker/tasks/s3_lifecycle/`):
