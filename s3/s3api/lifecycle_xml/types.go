...
// GetBucketLifecycleConfiguration returns. It lives outside s3/s3api so
// callers outside that package (shell, lifecycle worker) can parse and
// emit the same shape without pulling s3/s3api's full dependency graph
// — s3/s3api transitively imports s3/server, which imports s3/shell.
//
// Conversion to the engine-friendly canonical form
// (s3/s3api/s3lifecycle.Rule) lives next to the types here as
