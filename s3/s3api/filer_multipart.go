...
func extractMultipartSSES3Info(entry *filer_pb.Entry) (*multipartSSES3Info, error) {
	...
	if encryptionType := string(entry.Extended[s3_constants.HanzoSSES3Encryption]); encryptionType != s3_constants.SSEAlgorithmAES256 {
		return nil, nil
	}

	baseIVEncoded := entry.Extended[s3_constants.HanzoSSES3BaseIV]
	...
	keyDataEncoded := entry.Extended[s3_constants.HanzoSSES3KeyData]
...
// SSE-S3 attributes (HanzoSSES3Key / X-Amz-Server-Side-Encryption) onto a
...
func applyMultipartSSES3HeadersFromUploadEntry(dst *filer_pb.Entry, sses3Info *multipartSSES3Info) {
	...
	if _, exists := dst.Extended[s3_constants.HanzoSSES3Key]; !exists {
		dst.Extended[s3_constants.HanzoSSES3Key] = sses3Info.keyData
	}
...
func (s3a *S3ApiServer) prepareMultipartCompletionState(r *http.Request, input *s3.CompleteMultipartUploadInput, uploadDirectory, entryName, dirName string, completedPartNumbers []int, completedPartMap map[int][]string, maxPartNo int) (*multipartCompletionState, *CompleteMultipartUploadResult, s3err.ErrorCode) {
	if entry, err := s3a.resolveObjectEntry(*input.Bucket, *input.Key); err == nil && entry != nil && entry.Extended != nil {
		if uploadId, ok := entry.Extended[s3_constants.HanzoUploadId]; ok && *input.UploadId == string(uploadId) {
	...
	completionBody := func() s3err.ErrorCode {
		...
		if versioningState == s3_constants.VersioningEnabled {
			...
			if err := s3a.mkFile(versionDir, versionFileName, completionState.finalParts, func(versionEntry *filer_pb.Entry) {
				...
				versionEntry.Extended[s3_constants.HanzoUploadId] = []byte(*input.UploadId)
				// Store parts count for x-amz-mp-parts-count header
				versionEntry.Extended[s3_constants.HanzoMultipartPartsCount] = []byte(fmt.Sprintf("%d", len(completedPartNumbers)))
				...
				if partBoundariesJSON, err := json.Marshal(completionState.partBoundaries); err == nil {
					versionEntry.Extended[s3_constants.HanzoMultipartPartBoundaries] = partBoundariesJSON
				}
		...
		if versioningState == s3_constants.VersioningSuspended {
			...
			if err := s3a.writeMultipartObject(owner, routeKey, dirName, entryName, completionState.finalParts, func(entry *filer_pb.Entry) {
				...
				entry.Extended[s3_constants.HanzoUploadId] = []byte(*input.UploadId)
				// Store parts count for x-amz-mp-parts-count header
				entry.Extended[s3_constants.HanzoMultipartPartsCount] = []byte(fmt.Sprintf("%d", len(completedPartNumbers)))
				// Store part boundaries for GetObject with PartNumber
				if partBoundariesJSON, jsonErr := json.Marshal(completionState.partBoundaries); jsonErr == nil {
					entry.Extended[s3_constants.HanzoMultipartPartBoundaries] = partBoundariesJSON
				}
		...
		if err := s3a.writeMultipartObject(owner, routeKey, dirName, entryName, completionState.finalParts, func(entry *filer_pb.Entry) {
			...
			if completionState.entityWithTtl {
				entry.Extended[s3_constants.HanzoExpiresS3] = []byte("true")
			}
		}); err != nil {
...
func (s3a *S3ApiServer) applyMultipartEncryptionConfig(entry *filer_pb.Entry, config *MultipartEncryptionConfig) {
	...
	if config.IsSSEKMS {
		entry.Extended[s3_constants.HanzoSSEKMSKeyID] = []byte(config.KMSKeyID)
		if config.BucketKeyEnabled {
			entry.Extended[s3_constants.HanzoSSEKMSBucketKeyEnabled] = []byte("true")
		}
		if config.EncryptionContext != "" {
			entry.Extended[s3_constants.HanzoSSEKMSEncryptionContext] = []byte(config.EncryptionContext)
		}
		entry.Extended[s3_constants.HanzoSSEKMSBaseIV] = []byte(config.KMSBaseIVEncoded)
	...
	if config.IsSSES3 {
		entry.Extended[s3_constants.HanzoSSES3Encryption] = []byte(s3_constants.SSEAlgorithmAES256)
		entry.Extended[s3_constants.HanzoSSES3BaseIV] = []byte(config.S3BaseIVEncoded)
		entry.Extended[s3_constants.HanzoSSES3KeyData] = []byte(config.S3KeyDataEncoded)
