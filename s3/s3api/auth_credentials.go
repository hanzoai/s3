...
// Default account ID for all automated Hanzo accounts and fallback
...
func NewIdentityAccessManagementWithStore(option *S3ApiServerOption, filerClient *wdclient.FilerClient, explicitStore string) *IdentityAccessManagement {
	...
	// For "s3 mini" without any S3 config, default to allowing all access (isAuthEnabled = false)
...
// auth on and rejected every anonymous request to `docker run hanzo`
...
// hasAnyIdentity is a package-level signal for callers (like `s3 mini`'s
...
func (iam *IdentityAccessManagement) authenticateRequestInternal(r *http.Request) (*Identity, s3err.ErrorCode, authType) {
	...
	r.Header.Del(s3_constants.HanzoPrincipalHeader)
	r.Header.Del(s3_constants.HanzoSessionTokenHeader)
...
func buildPrincipalARN(identity *Identity, r *http.Request) string {
	...
	if r != nil {
		if principalARN := r.Header.Get(s3_constants.HanzoPrincipalHeader); principalARN != "" {
...
// startup behavior must be preserved — otherwise `docker run hanzo` (which
// starts `s3 mini` with no config) rejects every anonymous request.
//
...
func (iam *IdentityAccessManagement) authenticateJWTWithIAM(r *http.Request) (*Identity, s3err.ErrorCode) {
	...
	r.Header.Set(s3_constants.HanzoSessionTokenHeader, iamIdentity.SessionToken)
	r.Header.Set(s3_constants.HanzoPrincipalHeader, iamIdentity.Principal)
...
func (iam *IdentityAccessManagement) VerifyActionPermission(r *http.Request, identity *Identity, action Action, bucket, object string) s3err.ErrorCode {
	...
	hasSessionToken := r.Header.Get(s3_constants.HanzoSessionTokenHeader) != "" ||
...
func (iam *IdentityAccessManagement) authorizeWithIAM(r *http.Request, identity *Identity, action Action, bucket string, object string) s3err.ErrorCode {
	...
	// First check for JWT-based authentication headers (HanzoSessionTokenHeader)
	sessionToken := r.Header.Get(s3_constants.HanzoSessionTokenHeader)
	principal := r.Header.Get(s3_constants.HanzoPrincipalHeader)
