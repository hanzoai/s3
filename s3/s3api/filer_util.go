...
func (s3a *S3ApiServer) rm(parentDirectoryPath, entryName string, isDeleteData, isRecursive bool) error {
	...
	return s3a.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
...
func deleteObjectEntry(client filer_pb.HanzoFilerClient, parentDirectoryPath, entryName string, isDeleteData, isRecursive bool) error {
...
func doDeleteEntry(client filer_pb.HanzoFilerClient, parentDirectoryPath string, entryName string, isDeleteData bool, isRecursive bool) error {
...
func demoteDirectoryMarkerToImplicitDirectory(client filer_pb.HanzoFilerClient, parentDirectoryPath, entryName string) error {
...
func clearDirectoryMarkerMetadata(entry *filer_pb.Entry) {
	...
	for k, v := range entry.Extended {
		...
		if strings.HasPrefix(lowerKey, "xattr-") || strings.HasPrefix(lowerKey, s3_constants.HanzoInternalPrefix) {
			filtered[k] = v
		}
	}
