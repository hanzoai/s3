...
import (
	...
	s3_iam "github.com/hanzoai/s3/s3/iam"
...
func (iam *IdentityAccessManagement) validateSTSSessionToken(r *http.Request, sessionToken string, accessKey string) (*Identity, *Credential, s3err.ErrorCode) {
	...
	cred := &Credential{
		...
		Status:     s3_iam.AccessKeyStatusActive,
	...
	identity := &Identity{
		Name:         principal,
		Account:      &Account{Id: principal, DisplayName: sessionInfo.SessionName, EmailAddress: principal + "@hanzo.local"},
