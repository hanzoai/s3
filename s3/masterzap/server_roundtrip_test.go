...
// and hit by the real masterzap.New client, so the full loop is exercised end to
...
// over the masterzap SERVER adapter (NewServerBackend unary + NewStreamServer
// streaming), exactly as command/master.go now does, and returns the real
// masterzap client plus a cleanup func and the fake (for received-side asserts).
