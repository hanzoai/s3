...
func detectSSEType(entry *filer_pb.Entry) (filer_pb.SSEType, error) {
	...
	if entry.Extended != nil {
		hasS3 := len(entry.Extended[s3_constants.HanzoSSES3Key]) > 0
		hasKMS := len(entry.Extended[s3_constants.HanzoSSEKMSKey]) > 0
		hasC := len(entry.Extended[s3_constants.HanzoSSEIV]) > 0
		count := 0
		if hasS3 {
			count++
		}
...
func decryptSSEKMS(reader io.Reader, entry *filer_pb.Entry) (io.Reader, error) {
	...
	kmsMetadata := entry.Extended[s3_constants.HanzoSSEKMSKey]
