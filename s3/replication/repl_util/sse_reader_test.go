...
func TestDetectSSEType(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			entry: &filer_pb.Entry{
				Extended: map[string][]byte{
					s3_constants.HanzoSSES3Key: {0x01},
				},
			},
			wantType: filer_pb.SSEType_SSE_S3,
		},
		{
			...
			entry: &filer_pb.Entry{
				Extended: map[string][]byte{
					s3_constants.HanzoSSEKMSKey: {0x01},
				},
			},
			wantType: filer_pb.SSEType_SSE_KMS,
		},
		...
		},
		{
			...
			entry: &filer_pb.Entry{
				Extended: map[string][]byte{
					s3_constants.HanzoSSES3Key:  {0x01},
					s3_constants.HanzoSSEKMSKey: {0x02},
				},
			},
			wantError: true,
		},
...
func TestMaybeDecryptReader_SSES3(t *testing.T) {
	...
	entry := &filer_pb.Entry{
		Extended: map[string][]byte{
			s3_constants.HanzoSSES3Key: metadataBytes,
		},
	}
...
func TestMaybeDecryptReader_SSEKMS(t *testing.T) {
	...
	entry := &filer_pb.Entry{
		Extended: map[string][]byte{
			s3_constants.HanzoSSEKMSKey: metadataBytes,
		},
	}
