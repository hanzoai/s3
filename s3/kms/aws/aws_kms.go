...
import (
	...
	hanzokms "github.com/hanzoai/s3/s3/kms"
	"github.com/hanzoai/s3/s3/util"
)

func init() {
	// Register the AWS KMS provider
	hanzokms.RegisterProvider("aws", NewAWSKMSProvider)
}
...
func NewAWSKMSProvider(config util.Configuration) (hanzokms.KMSProvider, error) {
	if config == nil {
...
func (p *AWSKMSProvider) GenerateDataKey(ctx context.Context, req *hanzokms.GenerateDataKeyRequest) (*hanzokms.GenerateDataKeyResponse, error) {
	if req == nil {
	...
	envelopeBlob, err := hanzokms.CreateEnvelope("aws", actualKeyID, base64.StdEncoding.EncodeToString(result.CiphertextBlob), nil)
	if err != nil {
		return nil, fmt.Errorf("failed to create ciphertext envelope: %w", err)
	}

	response := &hanzokms.GenerateDataKeyResponse{
		KeyID:          actualKeyID,
...
func (p *AWSKMSProvider) Decrypt(ctx context.Context, req *hanzokms.DecryptRequest) (*hanzokms.DecryptResponse, error) {
	if req == nil {
	...
	envelope, err := hanzokms.ParseEnvelope(req.CiphertextBlob)
	if err != nil {
	...
	response := &hanzokms.DecryptResponse{
		KeyID:     keyID,
		Plaintext: result.Plaintext,
	}
...
func (p *AWSKMSProvider) DescribeKey(ctx context.Context, req *hanzokms.DescribeKeyRequest) (*hanzokms.DescribeKeyResponse, error) {
	if req == nil {
	...
	response := &hanzokms.DescribeKeyResponse{
	...
	if metadata.KeyUsage != nil {
		...
		case "ENCRYPT_DECRYPT":
			response.KeyUsage = hanzokms.KeyUsageEncryptDecrypt
		case "GENERATE_DATA_KEY":
			response.KeyUsage = hanzokms.KeyUsageGenerateDataKey
		}
	}
	...
	if metadata.KeyState != nil {
		...
		case "Enabled":
			response.KeyState = hanzokms.KeyStateEnabled
		case "Disabled":
			response.KeyState = hanzokms.KeyStateDisabled
		case "PendingDeletion":
			response.KeyState = hanzokms.KeyStatePendingDeletion
		case "Unavailable":
			response.KeyState = hanzokms.KeyStateUnavailable
		}
	}
	...
	if metadata.Origin != nil {
		...
		case "AWS_KMS":
			response.Origin = hanzokms.KeyOriginAWS
		case "EXTERNAL":
			response.Origin = hanzokms.KeyOriginExternal
		case "AWS_CLOUDHSM":
			response.Origin = hanzokms.KeyOriginCloudHSM
		}
	}
...
func (p *AWSKMSProvider) GetKeyID(ctx context.Context, keyIdentifier string) (string, error) {
	...
	descReq := &hanzokms.DescribeKeyRequest{KeyID: keyIdentifier}
...
func (p *AWSKMSProvider) convertAWSError(err error, keyID string) error {
	if awsErr, ok := err.(awserr.Error); ok {
		...
		case "NotFoundException":
			return &hanzokms.KMSError{
				Code:    hanzokms.ErrCodeNotFoundException,
				Message: awsErr.Message(),
				KeyID:   keyID,
			}
		case "DisabledException", "KeyUnavailableException":
			return &hanzokms.KMSError{
				Code:    hanzokms.ErrCodeKeyUnavailable,
				Message: awsErr.Message(),
				KeyID:   keyID,
			}
		case "AccessDeniedException":
			return &hanzokms.KMSError{
				Code:    hanzokms.ErrCodeAccessDenied,
				Message: awsErr.Message(),
				KeyID:   keyID,
			}
		case "InvalidKeyUsageException":
			return &hanzokms.KMSError{
				Code:    hanzokms.ErrCodeInvalidKeyUsage,
				Message: awsErr.Message(),
				KeyID:   keyID,
			}
		case "InvalidCiphertextException":
			return &hanzokms.KMSError{
				Code:    hanzokms.ErrCodeInvalidCiphertext,
				Message: awsErr.Message(),
				KeyID:   keyID,
			}
		case "KMSInternalException", "KMSInvalidStateException":
			return &hanzokms.KMSError{
				Code:    hanzokms.ErrCodeKMSInternalFailure,
				Message: awsErr.Message(),
				KeyID:   keyID,
			}
		default:
