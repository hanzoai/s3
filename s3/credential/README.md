...
This document shows how the credential store has been integrated into Hanzo's S3 API and IAM API components.

## Quick Start

1. **Generate credential configuration:**
   ```bash
   s3 scaffold -config=credential -output=.
   ```
...
- `./credential.toml`
- `$HOME/.s3/credential.toml`
- `/etc/hanzo/credential.toml`
...
1. **Easy Configuration** - Generate template with `s3 scaffold -config=credential`
