...
func healStrandedPointer(ctx context.Context, client filer_pb.HanzoFilerClient, parentPath util.FullPath, entry *filer_pb.Entry) error {
	if entry.Extended == nil {
		return nil
	}
...
// filerClientWrapper adapts a raw HanzoFilerClient to the
...
func (w *filerClientWrapper) WithFilerClient(streamingMode bool, fn func(filer_pb.HanzoFilerClient) error) error {
	return fn(w.client)
}
