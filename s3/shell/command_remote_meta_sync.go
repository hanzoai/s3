...
When "s3 filer.remote.sync" to upload local changes to remote, the criteria is:

	Attributes.Mtime > remoteEntry.RemoteMtime

Right after "s3 filer.remote.sync", the entry.RemoteEntry will be
...
// whose listing carries no mode. It matches what Hanzo S3 assigns native
