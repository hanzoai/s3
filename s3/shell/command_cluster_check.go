...
func (c *commandClusterCheck) Do(args []string, commandEnv *CommandEnv, writer io.Writer) (err error) {
	...
	for _, master := range masters {
		for _, volumeServer := range volumeServers {
			...
			err := pb.WithMasterClient(context.Background(), false, master, commandEnv.option.GrpcDialOption, false, func(client master_pb.HanzoClient) error {
	...
	for _, sourceMaster := range masters {
		for _, targetMaster := range masters {
			...
			err := pb.WithMasterClient(context.Background(), false, sourceMaster, commandEnv.option.GrpcDialOption, false, func(client master_pb.HanzoClient) error {
				pong, err := client.Ping(context.Background(), &master_pb.PingRequest{
	...
	for _, filer := range filers {
		for _, master := range masters {
			...
			err := pb.WithFilerClient(false, 0, filer, commandEnv.option.GrpcDialOption, func(client filer_pb.HanzoFilerClient) error {
	...
	for _, sourceFiler := range filers {
		for _, targetFiler := range filers {
			...
			err := pb.WithFilerClient(false, 0, sourceFiler, commandEnv.option.GrpcDialOption, func(client filer_pb.HanzoFilerClient) error {
				pong, err := client.Ping(context.Background(), &filer_pb.PingRequest{
