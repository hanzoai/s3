...
func (fs *FilerServer) maybeCheckJwtAuthorization(r *http.Request, isWrite bool) bool {
	...
	token, err := security.DecodeJwt(signingKey, tokenStr, &security.HanzoFilerClaims{})
	if err != nil {
	...
	claims, ok := token.Claims.(*security.HanzoFilerClaims)
	if !ok {
		glog.V(1).Infof("jwt claims not of type *HanzoFilerClaims from %s", r.RemoteAddr)
		return false
	}
