...
func (ms *MasterServer) SendHeartbeat(stream master_pb.Hanzo_SendHeartbeatServer) error {
	...
	for {
		...
		if dn == nil {
			// Skip delta heartbeat for volume server versions  better than 3.28 https://github.com/hanzoai/s3/pull/3630
			if heartbeat.Ip == "" {
				continue
...
func (ms *MasterServer) KeepConnected(stream master_pb.Hanzo_KeepConnectedServer) error {
