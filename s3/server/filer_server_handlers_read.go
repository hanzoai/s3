...
func (fs *FilerServer) GetOrHeadHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Filter out xattr-* (filesystem extended attributes) and internal Hanzo headers
	...
	//Hanzo custom header are not visible to Vue or javascript
	hanzoHeaders := []string{}
	for header := range w.Header() {
		if strings.HasPrefix(header, "Hanzo-") {
			hanzoHeaders = append(hanzoHeaders, header)
		}
	}
	hanzoHeaders = append(hanzoHeaders, "Content-Disposition")
	w.Header().Set("Access-Control-Expose-Headers", strings.Join(hanzoHeaders, ","))
