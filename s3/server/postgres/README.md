...
This package implements PostgreSQL wire protocol support for Hanzo, enabling universal compatibility with PostgreSQL clients, tools, and applications.
...
- **Data Type Mapping**: Hanzo to PostgreSQL type conversion
...
- **MQ Integration**: Direct integration with Hanzo SQL engine for real topic data
...
The PostgreSQL server now directly integrates with Hanzo Message Queue topics, providing:
...
- **Consistent SQL Engine**: Uses the same SQL engine as `s3-sql` command
...
### Import the Package
```go
import "github.com/hanzoai/s3/s3/server/postgres"
```
...
- **Command Documentation**: `s3-db -help`
...
**✅ POSTGRESQL ONLY**: Hanzo SQL engine exclusively supports PostgreSQL syntax:
...
- Type mapping between PostgreSQL and Hanzo schema types
- Error code mapping to PostgreSQL standards
- Comprehensive PostgreSQL wire protocol support

This package provides enterprise-grade PostgreSQL compatibility, enabling seamless integration of Hanzo with the entire PostgreSQL ecosystem.
