...
import (
	...
	"github.com/hanzoai/s3/s3/query/sqltypes"
	"github.com/hanzoai/s3/s3/util/sqlutil"
	"github.com/hanzoai/s3/s3/util/version"
)

// mapErrorToPostgreSQLCode maps Hanzo SQL engine errors to appropriate PostgreSQL error codes
...
func (s *PostgreSQLServer) handleSystemQuery(session *PostgreSQLSession, query string) *SystemQueryResult {
	...
	case "select version()":
		return &SystemQueryResult{
			...
			Rows:    [][]string{{fmt.Sprintf("Hanzo S3 %s (PostgreSQL 14.0 compatible)", version.VERSION_NUMBER)}},
		}
	...
	case "select current_setting('server_version')":
		return &SystemQueryResult{
			...
			Rows:    [][]string{{fmt.Sprintf("%s (Hanzo)", version.VERSION_NUMBER)}},
		}
...
// mapSchemaTypeToPostgreSQL maps Hanzo schema types to PostgreSQL type OIDs
