...
// carries no peer address (unlike gRPC's peer.FromContext). On that transport the
// caller is authenticated at the TRANSPORT by PQ-mTLS — pb.ServerTLSConfig
// requires and verifies the client cert against the CA and the allowed-CN gate,
// and fails closed without a CA. The legacy IP whitelist was a gRPC-era fallback
// that has no source IP to match over ZAP, so the transport's mTLS is the
// authorization boundary here. (When no guard is configured the call is a local /
// dev path and is likewise allowed.)
...
func (vs *VolumeServer) notifyMasterVolumeReadonly(ctx context.Context, v *storage.Volume, isReadOnly bool) error {
	if grpcErr := pb.WithMasterClient(context.Background(), false, vs.GetMaster(ctx), vs.grpcDialOption, false, func(client master_pb.HanzoClient) error {
...
func (vs *VolumeServer) Ping(ctx context.Context, req *volume_server_pb.PingRequest) (resp *volume_server_pb.PingResponse, pingErr error) {
	...
	if req.TargetType == cluster.MasterType {
		pingErr = pb.WithMasterClient(context.Background(), false, pb.ServerAddress(req.Target), vs.grpcDialOption, false, func(client master_pb.HanzoClient) error {
