...
func handleStaticResources(defaultMux *http.ServeMux) {
	...
	defaultMux.Handle("/hanzostatic/", http.StripPrefix("/hanzostatic", http.FileServer(http.FS(StaticFS))))
}

func handleStaticResources2(r *mux.Router) {
	r.Handle("/favicon.ico", http.FileServer(http.FS(StaticFS)))
	r.PathPrefix("/hanzostatic/").Handler(http.StripPrefix("/hanzostatic", http.FileServer(http.FS(StaticFS))))
}
