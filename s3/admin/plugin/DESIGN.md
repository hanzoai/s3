...
- Added `s3 worker` command as an external `plugin.proto` worker process.
...
Run example:
- Start admin: `s3 admin -master=localhost:9333`
- Start worker: `s3 worker -admin=localhost:23646`
- Optional explicit job type: `s3 worker -admin=localhost:23646 -jobType=vacuum`
- Optional stable worker ID persistence: `s3 worker -admin=localhost:23646 -workingDir=/var/lib/hanzo-plugin`
