...
// FUSE/VFS mount: the Go `s3 mount` ("mount") and the Rust VFS ("sw-vfs").
...
func (s *AdminServer) GetMountClients() (*MountClientsData, error) {
	...
	for _, filerAddr := range s.GetAllFilers() {
		...
		go func(filerAddr string) {
			...
			defer cancel()
			err := pb.WithGrpcFilerClient(false, 0, pb.ServerAddress(filerAddr), s.grpcDialOption, func(client filer_pb.HanzoFilerClient) error {
