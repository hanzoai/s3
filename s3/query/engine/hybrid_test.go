...
func TestSQLEngine_HybridSelectBasic(t *testing.T) {
	...
	if !hasSourceColumn {
		t.Skip("_source column not available in fallback mode - test requires real Hanzo cluster")
	}
...
func TestSQLEngine_HybridDataSource(t *testing.T) {
	...
	if sourceColumnIndex == -1 {
		t.Skip("Could not find _source column - test requires real Hanzo cluster")
	}
