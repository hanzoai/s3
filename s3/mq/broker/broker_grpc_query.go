...
func (b *MessageQueueBroker) GetUnflushedMessages(req *mq_pb.GetUnflushedMessagesRequest, stream mq_pb.HanzoMessaging_GetUnflushedMessagesServer) error {
...
func (b *MessageQueueBroker) buildBufferStartDeduplicationMap(partitionDir string) ([]BufferRange, error) {
	...
	err := b.fca.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
		var lastFileName string
		var hasMore = true

		for hasMore {
			var currentBatchProcessed int
			err := filer_pb.HanzoList(context.Background(), client, partitionDir, "", func(entry *filer_pb.Entry, isLast bool) error {
...
func (b *MessageQueueBroker) findBrokerForTopicPartition(topic *schema_pb.Topic, partition *schema_pb.Partition) (string, error) {
	...
	if !b.isLockOwner() {
		...
		err = b.withBrokerClient(false, balancerAddress, func(client mq_pb.HanzoMessagingClient) error {
...
func (b *MessageQueueBroker) redirectGetUnflushedMessages(brokerHost string, req *mq_pb.GetUnflushedMessagesRequest, stream mq_pb.HanzoMessaging_GetUnflushedMessagesServer) error {
	ctx := stream.Context()

	// Connect to the target broker and forward the request
	return b.withBrokerClient(false, pb.ServerAddress(brokerHost), func(client mq_pb.HanzoMessagingClient) error {
