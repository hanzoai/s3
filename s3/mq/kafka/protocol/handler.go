...
import (
	...
	"github.com/hanzoai/s3/s3/mq/kafka/consumer"
	"github.com/hanzoai/s3/s3/mq/kafka/consumer_offset"
	"github.com/hanzoai/s3/s3/mq/kafka/integration"
	"github.com/hanzoai/s3/s3/mq/kafka/schema"
	mqschema "github.com/hanzoai/s3/s3/mq/schema"
...
const (
	// DefaultKafkaNamespace is the default namespace for Kafka topics in HanzoMQ
...
// HanzoMQHandlerInterface defines the interface for HanzoMQ integration
type HanzoMQHandlerInterface interface {
	...
	// WithFilerClient executes a function with a filer client for accessing HanzoMQ metadata
	WithFilerClient(streamingMode bool, fn func(client filer_pb.HanzoFilerClient) error) error
...
// Handler processes Kafka protocol requests from clients using HanzoMQ
type Handler struct {
	// HanzoMQ integration
	hanzoMQHandler HanzoMQHandlerInterface
...
// For production use with persistent storage, use NewHanzoMQBrokerHandler instead
func NewHandler() *Handler {
	...
	panic("NewHandler() with in-memory storage should NEVER be used in production! Use NewHanzoMQBrokerHandler() with HanzoMQ masters for production, or NewTestHandler() for tests.")
}
...
// NewTestHandlerWithMock creates a test handler with a custom HanzoMQHandlerInterface
// This is useful for unit tests that need a handler but don't want to connect to real HanzoMQ
func NewTestHandlerWithMock(mockHandler HanzoMQHandlerInterface) *Handler {
	return &Handler{
		hanzoMQHandler:      mockHandler,
...
// NewHanzoMQBrokerHandler creates a new handler with HanzoMQ broker integration
func NewHanzoMQBrokerHandler(masters string, filerGroup string, clientHost string) (*Handler, error) {
	return NewHanzoMQBrokerHandlerWithDefaults(masters, filerGroup, clientHost, 4) // Default to 4 partitions
}

// NewHanzoMQBrokerHandlerWithDefaults creates a new handler with HanzoMQ broker integration and custom defaults
func NewHanzoMQBrokerHandlerWithDefaults(masters string, filerGroup string, clientHost string, defaultPartitions int32) (*Handler, error) {
	// Set up HanzoMQ integration
	smqHandler, err := integration.NewHanzoMQBrokerHandler(masters, filerGroup, clientHost)
	if err != nil {
		return nil, err
	}

	// Use the shared filer client accessor from HanzoMQHandler
...
// This delegates to the underlying HanzoMQ handler
func (h *Handler) AddTopicForTesting(topicName string, partitions int32) {
	if h.hanzoMQHandler != nil {
		h.hanzoMQHandler.CreateTopic(topicName, partitions)
	}
}

// Delegate methods to HanzoMQ handler
...
func (h *Handler) Close() error {
	...
	// Close HanzoMQ handler if present
	if h.hanzoMQHandler != nil {
		return h.hanzoMQHandler.Close()
	}
	return nil
}
...
func (h *Handler) GetSMQBrokerAddresses() []string {
	// First try to get from the HanzoMQ handler (preferred)
	if h.hanzoMQHandler != nil {
		if brokerAddresses := h.hanzoMQHandler.GetBrokerAddresses(); len(brokerAddresses) > 0 {
...
func (h *Handler) HandleConn(ctx context.Context, conn net.Conn) error {
	...
	connBrokerClient, err := h.hanzoMQHandler.CreatePerConnectionBrokerClient()
	if err != nil {
...
func (h *Handler) HandleMetadataV0(correlationID uint32, requestBody []byte) ([]byte, error) {
	...
	// Determine topics to return using HanzoMQ handler
	var topicsToReturn []string
	if len(requestedTopics) == 0 {
		topicsToReturn = h.hanzoMQHandler.ListTopics()
	} else {
	...
	for _, topicName := range topicsToReturn {
		...
		topicInfo, exists := h.hanzoMQHandler.GetTopicInfo(topicName)
	...
	for i := uint32(0); i < topicsCount && offset < len(requestBody); i++ {
		...
		for j := uint32(0); j < partitionsCount && offset+12 <= len(requestBody); j++ {
			...
			case -2: // earliest offset
				...
				earliestOffset, err := h.hanzoMQHandler.GetEarliestOffset(string(topicName), int32(partitionID))
				if err != nil {
			...
			case -1: // latest offset
				...
				if h.hanzoMQHandler == nil {
					...
					latestOffset, err := h.hanzoMQHandler.GetLatestOffset(string(topicName), int32(partitionID))
					if err != nil {
						responseOffset = 0 // fallback to 0
					} else {
...
func (h *Handler) handleCreateTopicsV2To4(correlationID uint32, requestBody []byte) ([]byte, error) {
	...
	for _, t := range topics {
		...
		if h.hanzoMQHandler.TopicExists(t.name) {
...
func (h *Handler) handleCreateTopicsV0V1(correlationID uint32, requestBody []byte) ([]byte, error) {
	...
	for i := uint32(0); i < topicsCount && offset < len(requestBody); i++ {
		...
		// Use HanzoMQ integration
		if h.hanzoMQHandler.TopicExists(topicName) {
			errorCode = 36 // TOPIC_ALREADY_EXISTS
		} else {
			// Create the topic in HanzoMQ with schema support
...
func (h *Handler) handleDeleteTopics(correlationID uint32, requestBody []byte) ([]byte, error) {
	...
	// Process each topic (using HanzoMQ handler)
	...
	for i := uint32(0); i < topicsCount && offset < len(requestBody); i++ {
		...
		if !h.hanzoMQHandler.TopicExists(topicName) {
			...
			// Delete the topic from HanzoMQ
			if err := h.hanzoMQHandler.DeleteTopic(topicName); err != nil {
				errorCode = 0xFFFF // UNKNOWN_SERVER_ERROR (-1 as uint16)
...
func (h *Handler) registerSchemasViaBrokerAPI(topicName string, valueRecordType *schema_pb.RecordType, keyRecordType *schema_pb.RecordType) error {
	...
	// Require HanzoMQ integration to access broker
	if h.hanzoMQHandler == nil {
		return fmt.Errorf("no HanzoMQ handler available for broker access")
	}

	// Get broker addresses
	brokerAddresses := h.hanzoMQHandler.GetBrokerAddresses()
	...
	return pb.WithBrokerGrpcClient(false, brokerAddress, grpcDialOption, func(client mq_pb.HanzoMessagingClient) error {
...
func (h *Handler) ensureTopicExists(topicName string, partitions int32) bool {
	...
	h.hanzoMQHandler.InvalidateTopicExistsCache(topicName)
	if h.hanzoMQHandler.TopicExists(topicName) {
		return true
	}
...
func (h *Handler) createTopicWithSchemaSupport(topicName string, partitions int32) error {
	...
	if h.schemaRegistryURL != "" {
		...
		if keyRecordType != nil || valueRecordType != nil {
			...
			return h.hanzoMQHandler.CreateTopicWithSchemas(topicName, partitions, keyRecordType, valueRecordType)
		}
	...
	return h.hanzoMQHandler.CreateTopic(topicName, partitions)
}
