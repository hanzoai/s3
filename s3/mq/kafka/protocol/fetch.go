...
func (h *Handler) handleFetch(ctx context.Context, correlationID uint32, apiVersion uint16, requestBody []byte) ([]byte, error) {
	...
	allTopicsExist := func() bool {
		for _, topic := range fetchRequest.Topics {
			if !h.hanzoMQHandler.TopicExists(topic.Name) {
				return false
			}
		}
		return true
	...
	hasDataAvailable := func() bool {
		...
		for _, topic := range fetchRequest.Topics {
			...
			for _, partition := range topic.Partitions {
				hwm, err := h.hanzoMQHandler.GetLatestOffset(topic.Name, partition.PartitionID)
				if err != nil {
...
// constructRecordBatchFromSMQ creates a Kafka record batch from HanzoMQ records
