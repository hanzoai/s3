...
func (pr *partitionReader) serveFetchRequest(ctx context.Context, req *partitionFetchRequest) {
	...
	// Get high water mark
	hwmUnknown := false
	hwm, hwmErr := pr.handler.hanzoMQHandler.GetLatestOffset(pr.topicName, pr.partitionID)
	if hwmErr != nil {
...
func (pr *partitionReader) readRecords(ctx context.Context, fromOffset int64, maxBytes int32, maxWaitMs int32, highWaterMark int64) ([]byte, int64) {
	...
	smqRecords, err := pr.handler.hanzoMQHandler.GetStoredRecords(fallbackCtx, pr.topicName, pr.partitionID, fromOffset, 10)
