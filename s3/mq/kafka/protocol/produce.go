...
import (
	...
	"github.com/hanzoai/s3/s3/mq/kafka/compression"
		...
		for j := uint32(0); j < partitionsCount && offset < len(requestBody); j++ {
			...
			if !topicExists {
				...
				if parseErr != nil {
					...
					offset, err := h.produceToHanzoMQ(ctx, topicName, int32(partitionID), recordSetData)
					if err != nil {
...
// produceToHanzoMQ publishes a single record to HanzoMQ (simplified for Phase 2)
// ctx controls the publish timeout - if client cancels, produce operation is cancelled
func (h *Handler) produceToHanzoMQ(ctx context.Context, topic string, partition int32, recordSetData []byte) (int64, error) {
...
func (h *Handler) produceSchemaBasedRecord(ctx context.Context, topic string, partition int32, key []byte, value []byte) (int64, error) {
	...
	if h.isSystemTopic(topic) {
		offset, err := h.hanzoMQHandler.ProduceRecord(ctx, topic, partition, key, value)
		return offset, err
	}
	...
	if !isEnabled {
		return h.hanzoMQHandler.ProduceRecord(ctx, topic, partition, key, value)
	}
	...
	if valueDecodedMsg != nil || keyDecodedMsg != nil {
		...
		return h.hanzoMQHandler.ProduceRecordValue(ctx, topic, partition, finalKey, recordValueBytes)
	} else {
		// Send with raw format for non-schematized data
		return h.hanzoMQHandler.ProduceRecord(ctx, topic, partition, finalKey, recordValueBytes)
	}
}
