...
// AvroDecoder handles Avro schema decoding and conversion to HanzoMQ format
...
// DecodeToRecordValue decodes Avro data directly to HanzoMQ RecordValue
...
// InferRecordType infers a HanzoMQ RecordType from an Avro schema
func (ad *AvroDecoder) InferRecordType() (*schema_pb.RecordType, error) {
	schema := ad.codec.Schema()
	return avroSchemaToRecordType(schema)
}

// MapToRecordValue converts a Go map to HanzoMQ RecordValue
...
// goValueToSchemaValue converts a Go value to a HanzoMQ Value
...
// avroSchemaToRecordType converts an Avro schema to HanzoMQ RecordType
...
func extractAvroFields(avroSchema map[string]interface{}) ([]*schema_pb.Field, error) {
	...
	// Convert each Avro field to HanzoMQ field
	...
	for i, fieldInterface := range fieldsArray {
		...
		field, err := convertAvroFieldToHanzoMQ(fieldMap, int32(i))
		if err != nil {
...
// convertAvroFieldToHanzoMQ converts a single Avro field to HanzoMQ Field
func convertAvroFieldToHanzoMQ(avroField map[string]interface{}, fieldIndex int32) (*schema_pb.Field, error) {
	// Extract field name
	...
	fieldType, isRepeated, err := convertAvroTypeToHanzoMQWithRepeated(avroField["type"])
	if err != nil {
...
// convertAvroTypeToHanzoMQ converts Avro type to HanzoMQ Type
func convertAvroTypeToHanzoMQ(avroType interface{}) (*schema_pb.Type, error) {
	fieldType, _, err := convertAvroTypeToHanzoMQWithRepeated(avroType)
	return fieldType, err
}

// convertAvroTypeToHanzoMQWithRepeated converts Avro type to HanzoMQ Type and returns if it's repeated
func convertAvroTypeToHanzoMQWithRepeated(avroType interface{}) (*schema_pb.Type, bool, error) {
...
// convertAvroSimpleType converts simple Avro types to HanzoMQ types
...
// convertAvroComplexTypeWithRepeated converts complex Avro types to HanzoMQ types and returns if it's repeated
func convertAvroComplexTypeWithRepeated(avroType map[string]interface{}) (*schema_pb.Type, bool, error) {
	...
	case "array":
		// Array type
		itemsType, err := convertAvroTypeToHanzoMQ(avroType["items"])
		if err != nil {
...
func convertAvroSimpleTypeWithLogical(primitiveType string, avroType map[string]interface{}) (*schema_pb.Type, bool, error) {
	...
	// Map logical types to appropriate HanzoMQ types
	switch logicalType {
	case "decimal":
...
// convertAvroUnionType converts Avro union types to HanzoMQ types
func convertAvroUnionType(unionTypes []interface{}) (*schema_pb.Type, error) {
	...
	for _, unionType := range unionTypes {
		...
		return convertAvroTypeToHanzoMQ(unionType)
	}
...
// inferTypeFromValue infers a HanzoMQ Type from a Go value
