...
// GetStoredRecords retrieves records from HanzoMQ using the proper subscriber API
// ctx controls the fetch timeout (should match Kafka fetch request's MaxWaitTime)
func (h *HanzoMQHandler) GetStoredRecords(ctx context.Context, topic string, partition int32, fromOffset int64, maxRecords int) ([]SMQRecord, error) {
	...
	hanzoRecords, err := brokerClient.FetchMessagesStateless(ctx, topic, partition, fromOffset, maxRecords, consumerGroup, consumerID)
	if err != nil {
		glog.Errorf("[FETCH-STATELESS] Failed to fetch records: %v", err)
		return nil, fmt.Errorf("failed to fetch records: %v", err)
	}

	glog.V(4).Infof("[FETCH-STATELESS] Fetched %d records", len(hanzoRecords))
	//
	...
	// Convert HanzoMQ records to SMQRecord interface with proper Kafka offsets
	smqRecords := make([]SMQRecord, 0, len(hanzoRecords))
	for i, hanzoRecord := range hanzoRecords {
		// CRITICAL FIX: Use the actual offset from HanzoMQ
		// The HanzoRecord.Offset field now contains the correct offset from the subscriber
		kafkaOffset := hanzoRecord.Offset
		...
		smqRecord := &HanzoSMQRecord{
			key:       hanzoRecord.Key,
			value:     hanzoRecord.Value,
			timestamp: hanzoRecord.Timestamp,
			offset:    kafkaOffset,
		}
		smqRecords = append(smqRecords, smqRecord)

		glog.V(4).Infof("[FETCH] Record %d: offset=%d, keyLen=%d, valueLen=%d", i, kafkaOffset, len(hanzoRecord.Key), len(hanzoRecord.Value))
	}
...
func (h *HanzoMQHandler) GetEarliestOffset(topic string, partition int32) (int64, error) {
...
// ALWAYS queries SMQ broker directly - no ledger involved
func (h *HanzoMQHandler) GetLatestOffset(topic string, partition int32) (int64, error) {
	// Check if topic exists
	if !h.TopicExists(topic) {
...
func (h *HanzoMQHandler) WithFilerClient(streamingMode bool, fn func(client filer_pb.HanzoFilerClient) error) error {
	if h.brokerClient == nil {
...
func (h *HanzoMQHandler) GetFilerAddress() string {
	if h.brokerClient != nil {
...
// ProduceRecord publishes a record to HanzoMQ and lets SMQ generate the offset
// ctx controls the publish timeout - if client cancels, broker operation is cancelled
func (h *HanzoMQHandler) ProduceRecord(ctx context.Context, topic string, partition int32, key []byte, value []byte) (int64, error) {
	if len(key) > 0 {
	}
	if len(value) > 0 {
	} else {
	...
	// Publish to HanzoMQ and let SMQ generate the offset
	var smqOffset int64
	...
	if publishErr != nil {
		return 0, fmt.Errorf("failed to publish to HanzoMQ: %v", publishErr)
	}
...
// ProduceRecordValue produces a record using RecordValue format to HanzoMQ
// ALWAYS uses broker's assigned offset - no ledger involved
// ctx controls the publish timeout - if client cancels, broker operation is cancelled
func (h *HanzoMQHandler) ProduceRecordValue(ctx context.Context, topic string, partition int32, key []byte, recordValueBytes []byte) (int64, error) {
	// Verify topic exists
	if !h.TopicExists(topic) {
		return 0, fmt.Errorf("topic %s does not exist", topic)
	}
	...
	// Publish RecordValue to HanzoMQ and get the broker-assigned offset
	var smqOffset int64
	var publishErr error
	if h.brokerClient == nil {
		publishErr = fmt.Errorf("no broker client available")
	...
	if publishErr != nil {
		return 0, fmt.Errorf("failed to publish RecordValue to HanzoMQ: %v", publishErr)
	}
...
func (h *HanzoMQHandler) FetchRecords(topic string, partition int32, fetchOffset int64, maxBytes int32) ([]byte, error) {
	// Verify topic exists
	if !h.TopicExists(topic) {
		return nil, fmt.Errorf("topic %s does not exist", topic)
	}
	...
	hanzoRecords, err = h.brokerClient.ReadRecordsFromOffset(context.Background(), brokerSubscriber, fetchOffset, recordsToFetch)
	...
	// Map HanzoMQ records to Kafka offsets and update ledger
	kafkaRecords, err := h.mapHanzoToKafkaOffsets(topic, partition, hanzoRecords, fetchOffset)
	if err != nil {
		return nil, fmt.Errorf("failed to map offsets: %v", err)
	}

	// Convert mapped records to Kafka record batch format
	return h.convertHanzoToKafkaRecordBatch(kafkaRecords, fetchOffset, maxBytes)
}

// mapHanzoToKafkaOffsets maps HanzoMQ records to proper Kafka offsets
func (h *HanzoMQHandler) mapHanzoToKafkaOffsets(topic string, partition int32, hanzoRecords []*HanzoRecord, startOffset int64) ([]*HanzoRecord, error) {
	if len(hanzoRecords) == 0 {
		return hanzoRecords, nil
	}

	// DEPRECATED: This function only used in old tests
	// Just map offsets sequentially
	mappedRecords := make([]*HanzoRecord, 0, len(hanzoRecords))

	for i, hanzoRecord := range hanzoRecords {
		currentKafkaOffset := startOffset + int64(i)

		// Create a copy of the record with proper Kafka offset assignment
		mappedRecord := &HanzoRecord{
			Key:       hanzoRecord.Key,
			Value:     hanzoRecord.Value,
			Timestamp: hanzoRecord.Timestamp,
...
// convertHanzoToKafkaRecordBatch converts HanzoMQ records to Kafka record batch format
func (h *HanzoMQHandler) convertHanzoToKafkaRecordBatch(hanzoRecords []*HanzoRecord, fetchOffset int64, maxBytes int32) ([]byte, error) {
	if len(hanzoRecords) == 0 {
		return []byte{}, nil
	}
	...
	// Last offset delta
	lastOffsetDelta := uint32(len(hanzoRecords) - 1)
	lastOffsetDeltaBytes := make([]byte, 4)
	binary.BigEndian.PutUint32(lastOffsetDeltaBytes, lastOffsetDelta)
	batch = append(batch, lastOffsetDeltaBytes...)

	// Timestamps - use actual timestamps from HanzoMQ records
	var firstTimestamp, maxTimestamp int64
	if len(hanzoRecords) > 0 {
		firstTimestamp = hanzoRecords[0].Timestamp
		maxTimestamp = firstTimestamp
		for _, record := range hanzoRecords {
	...
	binary.BigEndian.PutUint32(recordCountBytes, uint32(len(hanzoRecords)))
	batch = append(batch, recordCountBytes...)

	// Add actual records from HanzoMQ
	for i, hanzoRecord := range hanzoRecords {
		record := h.convertSingleHanzoRecord(hanzoRecord, int64(i), fetchOffset)
...
// convertSingleHanzoRecord converts a single HanzoMQ record to Kafka format
func (h *HanzoMQHandler) convertSingleHanzoRecord(hanzoRecord *HanzoRecord, index, baseOffset int64) []byte {
	...
	timestampDelta := hanzoRecord.Timestamp - baseOffset // Simple delta calculation
	if timestampDelta < 0 {
		timestampDelta = 0
	}
	...
	// Key length and key
	if len(hanzoRecord.Key) > 0 {
		record = append(record, byte(len(hanzoRecord.Key)))
		record = append(record, hanzoRecord.Key...)
	} else {
		// Null key
	...
	if len(hanzoRecord.Value) > 0 {
		record = append(record, byte(len(hanzoRecord.Value)))
		record = append(record, hanzoRecord.Value...)
	} else {
		// Empty value
		record = append(record, 0)
	}
