...
// CreateTopic creates a new topic in both Kafka registry and HanzoMQ
func (h *HanzoMQHandler) CreateTopic(name string, partitions int32) error {
	return h.CreateTopicWithSchema(name, partitions, nil)
}

// CreateTopicWithSchema creates a topic with optional value schema
func (h *HanzoMQHandler) CreateTopicWithSchema(name string, partitions int32, recordType *schema_pb.RecordType) error {
...
func (h *HanzoMQHandler) CreateTopicWithSchemas(name string, partitions int32, keyRecordType *schema_pb.RecordType, valueRecordType *schema_pb.RecordType) error {
	// Check if topic already exists in filer
	if h.checkTopicInFiler(name) {
		return fmt.Errorf("%s: %w", name, ErrTopicAlreadyExists)
	}

	// Create HanzoMQ topic reference
	hanzoTopic := &schema_pb.Topic{
		Namespace: "kafka",
		Name:      name,
	}

	// Configure topic with HanzoMQ broker via gRPC
	if len(h.brokerAddresses) > 0 {
		...
		err := pb.WithBrokerGrpcClient(false, brokerAddress, grpcDialOption, func(client mq_pb.HanzoMessagingClient) error {
	...
	// Topic is now stored in filer only via HanzoMQ broker
...
func (h *HanzoMQHandler) CreateTopicWithRecordType(name string, partitions int32, flatSchema *schema_pb.RecordType, keyColumns []string) error {
	// Check if topic already exists in filer
	if h.checkTopicInFiler(name) {
		return fmt.Errorf("%s: %w", name, ErrTopicAlreadyExists)
	}
	...
	if len(h.brokerAddresses) > 0 {
		...
		glog.Warningf("No broker addresses configured, topic %s not created in HanzoMQ", name)
	}
...
// DeleteTopic removes a topic from both Kafka registry and HanzoMQ
func (h *HanzoMQHandler) DeleteTopic(name string) error {
	...
	// Topic removal from filer would be handled by HanzoMQ broker
...
func (h *HanzoMQHandler) TopicExists(name string) bool {
	// Check cache first
	...
	var exists bool
	// Check via HanzoMQ broker (includes in-memory topics)
	if h.brokerClient != nil {
		var err error
...
func (h *HanzoMQHandler) InvalidateTopicExistsCache(name string) {
	h.topicExistsCacheMu.Lock()
...
func (h *HanzoMQHandler) GetTopicInfo(name string) (*KafkaTopicInfo, bool) {
...
// ListTopics returns all topic names from HanzoMQ broker (includes in-memory topics)
func (h *HanzoMQHandler) ListTopics() []string {
	// Get topics from HanzoMQ broker (includes in-memory topics)
	if h.brokerClient != nil {
...
func (h *HanzoMQHandler) checkTopicInFiler(topicName string) bool {
	if h.filerClientAccessor == nil {
		return false
	}

	var exists bool
	h.filerClientAccessor.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
