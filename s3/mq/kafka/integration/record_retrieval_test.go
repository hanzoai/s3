...
// MockHanzoClient provides a mock implementation for testing
type MockHanzoClient struct {
	records map[string]map[int32][]*HanzoRecord // topic -> partition -> records
}

func NewMockHanzoClient() *MockHanzoClient {
	return &MockHanzoClient{
		records: make(map[string]map[int32][]*HanzoRecord),
	}
}

func (m *MockHanzoClient) AddRecord(topic string, partition int32, key []byte, value []byte, timestamp int64) {
	if m.records[topic] == nil {
		m.records[topic] = make(map[int32][]*HanzoRecord)
	}
	if m.records[topic][partition] == nil {
		m.records[topic][partition] = make([]*HanzoRecord, 0)
	}
...
func (m *MockHanzoClient) GetRecords(topic string, partition int32, fromOffset int64, maxRecords int) ([]*HanzoRecord, error) {
...
func TestHanzoSMQRecord_Interface(t *testing.T) {
	// Test that HanzoSMQRecord properly implements SMQRecord interface
	key := []byte("test-key")
	value := []byte("test-value")
	timestamp := time.Now().UnixNano()
	kafkaOffset := int64(42)

	record := &HanzoSMQRecord{
		key:       key,
		value:     value,
		timestamp: timestamp,
		offset:    kafkaOffset,
...
func TestHanzoMQHandler_GetStoredRecords_EmptyTopic(t *testing.T) {
...
func TestHanzoMQHandler_GetStoredRecords_EmptyPartition(t *testing.T) {
	// Note: Ledgers have been removed - SMQ broker handles all offset management directly
	// This test is now obsolete as GetStoredRecords requires a real broker connection
	t.Skip("Test obsolete: ledgers removed, SMQ broker handles offset management")
}

func TestHanzoMQHandler_GetStoredRecords_OffsetBeyondHighWaterMark(t *testing.T) {
	// Note: Ledgers have been removed - SMQ broker handles all offset management directly
	// This test is now obsolete as GetStoredRecords requires a real broker connection
	t.Skip("Test obsolete: ledgers removed, SMQ broker handles offset management")
}

func TestHanzoMQHandler_GetStoredRecords_MaxRecordsLimit(t *testing.T) {
	// Note: Ledgers have been removed - SMQ broker handles all offset management directly
	// This test is now obsolete as GetStoredRecords requires a real broker connection
	t.Skip("Test obsolete: ledgers removed, SMQ broker handles offset management")
}
...
func BenchmarkHanzoSMQRecord_GetMethods(b *testing.B) {
	record := &HanzoSMQRecord{
