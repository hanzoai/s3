...
func NewHanzoMQBrokerHandler(masters string, filerGroup string, clientHost string) (*HanzoMQHandler, error) {
	if masters == "" {
		return nil, fmt.Errorf("masters required - HanzoMQ infrastructure must be configured")
	}

	// Parse master addresses using Hanzo utilities
...
func discoverBrokersWithMasterClient(masterClient *wdclient.MasterClient, filerGroup string) ([]string, error) {
	...
	err := masterClient.WithClient(false, func(client master_pb.HanzoClient) error {
...
func (h *HanzoMQHandler) CreatePerConnectionBrokerClient() (*BrokerClient, error) {
