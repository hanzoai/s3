...
const (
	// ConsumerOffsetsBasePath is the base path for storing Kafka consumer offsets in Hanzo
...
// FilerStorage implements OffsetStorage using Hanzo filer
...
func (f *FilerStorage) writeFile(path string, data []byte) error {
	...
	return f.fca.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
		// Create entry
...
func (f *FilerStorage) readFile(path string) ([]byte, error) {
	...
	var data []byte
	err := f.fca.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
		// Get the entry
