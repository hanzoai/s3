...
type Options struct {
	Listen            string
	Masters           string // Hanzo master servers
...
func NewServer(opts Options) *Server {
	...
	// Create HanzoMQ handler - masters are required for production
	if opts.Masters == "" {
		glog.Fatalf("HanzoMQ masters are required for Kafka gateway - provide masters addresses")
	}
	...
	handler, err = protocol.NewHanzoMQBrokerHandler(opts.Masters, opts.FilerGroup, clientHost)
	if err != nil {
		glog.Fatalf("Failed to create HanzoMQ handler with masters %s: %v", opts.Masters, err)
	}

	glog.V(1).Infof("Created Kafka gateway with HanzoMQ brokers via masters %s", opts.Masters)
...
// This allows basic gateway functionality testing without requiring HanzoMQ masters
func NewTestServerForUnitTests(opts Options) *Server {
	ctx, cancel := context.WithCancel(context.Background())

	// Create a minimal handler with mock HanzoMQ backend
...
func (s *Server) Close() error {
	...
	// Close the handler (important for HanzoMQ mode)
	if s.handler != nil {
