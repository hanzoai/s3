...
// mockHanzoMQHandler is a stateful mock for unit testing without real HanzoMQ
type mockHanzoMQHandler struct {
	mu      sync.RWMutex
...
func newMockHanzoMQHandler() *mockHanzoMQHandler {
	return &mockHanzoMQHandler{
...
func (m *mockHanzoMQHandler) TopicExists(topic string) bool {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *mockHanzoMQHandler) ListTopics() []string {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *mockHanzoMQHandler) CreateTopic(topic string, partitions int32) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *mockHanzoMQHandler) CreateTopicWithSchemas(name string, partitions int32, keyRecordType *schema_pb.RecordType, valueRecordType *schema_pb.RecordType) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *mockHanzoMQHandler) DeleteTopic(topic string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *mockHanzoMQHandler) GetTopicInfo(topic string) (*integration.KafkaTopicInfo, bool) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *mockHanzoMQHandler) InvalidateTopicExistsCache(topic string) {
	// Mock handler doesn't cache topic existence, so this is a no-op
}

func (m *mockHanzoMQHandler) ProduceRecord(ctx context.Context, topicName string, partitionID int32, key, value []byte) (int64, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *mockHanzoMQHandler) ProduceRecordValue(ctx context.Context, topicName string, partitionID int32, key []byte, recordValueBytes []byte) (int64, error) {
	return m.ProduceRecord(ctx, topicName, partitionID, key, recordValueBytes)
}

func (m *mockHanzoMQHandler) GetStoredRecords(ctx context.Context, topic string, partition int32, fromOffset int64, maxRecords int) ([]integration.SMQRecord, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *mockHanzoMQHandler) GetEarliestOffset(topic string, partition int32) (int64, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *mockHanzoMQHandler) GetLatestOffset(topic string, partition int32) (int64, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *mockHanzoMQHandler) WithFilerClient(streamingMode bool, fn func(filer_pb.HanzoFilerClient) error) error {
	return fmt.Errorf("mock handler: not implemented")
}

func (m *mockHanzoMQHandler) CreatePerConnectionBrokerClient() (*integration.BrokerClient, error) {
...
func (m *mockHanzoMQHandler) GetFilerClientAccessor() *filer_client.FilerClientAccessor {
	return nil
}

func (m *mockHanzoMQHandler) GetBrokerAddresses() []string {
	return []string{"localhost:9092"} // Return a dummy broker address for unit tests
}

func (m *mockHanzoMQHandler) Close() error { return nil }

func (m *mockHanzoMQHandler) SetProtocolHandler(h integration.ProtocolHandler) {}
...
func NewMinimalTestHandler() *protocol.Handler {
	return protocol.NewTestHandlerWithMock(newMockHanzoMQHandler())
}
