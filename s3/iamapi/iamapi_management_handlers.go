...
// Common IAM types and helpers are imported from the shared s3/iam package.
...
import (
	...
	iamlib "github.com/hanzoai/s3/s3/iam"
...
// policyDefaultVersionId is the single managed-policy version Hanzo exposes.
...
// Hanzo keeps a single current document per managed policy (no version
...
func (iama *IamApiServer) CreatePolicyVersion(s3cfg *iam_pb.S3ApiConfiguration, values url.Values) (resp *CreatePolicyVersionResponse, iamError *IamError) {
	...
	// Hanzo stores a single, always-default managed policy version. On AWS,
	...
	if !strings.EqualFold(values.Get("SetAsDefault"), "true") {
		return resp, &IamError{Code: iam.ErrCodeInvalidInputException, Error: fmt.Errorf("SetAsDefault must be true: Hanzo stores a single managed policy version")}
	}
	policyDocument, err := GetPolicyDocument(&policyDocumentString)
	if err != nil {
		return resp, &IamError{Code: iam.ErrCodeMalformedPolicyDocumentException, Error: err}
...
// ListPolicyVersions lists the versions of a managed policy. Hanzo stores a
