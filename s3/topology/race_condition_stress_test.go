...
func TestRaceConditionStress(t *testing.T) {
	...
	topo := NewTopology("s3fs", sequence.NewMemorySequencer(), uint64(volumeLimitMB)*1024*1024, 5, false)
...
func TestCapacityJudgmentAccuracy(t *testing.T) {
	...
	topo := NewTopology("s3fs", sequence.NewMemorySequencer(), 5*1024*1024*1024, 5, false)
...
func TestReservationSystemPerformance(t *testing.T) {
	// Create topology
	topo := NewTopology("s3fs", sequence.NewMemorySequencer(), 32*1024, 5, false)
