...
func TestUpdateSigningKeysRotates(t *testing.T) {
	...
	if _, err := DecodeJwt(g.SigningKey(), tok, &HanzoFileIdClaims{}); err != nil {
		t.Fatalf("token minted with rotated key should verify: %v", err)
	}
	if _, err := DecodeJwt(SigningKey("old-write"), tok, &HanzoFileIdClaims{}); err == nil {
	...
	for w := 0; w < 8; w++ {
		...
		go func() {
			...
			for i := 0; i < 1000; i++ {
				...
				if _, err := DecodeJwt(key, tok, &HanzoFileIdClaims{}); err != nil {
