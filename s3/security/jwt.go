...
// HanzoFileIdClaims is created by Master server(s) and consumed by Volume server(s),
// restricting the access this JWT allows to only a single file.
type HanzoFileIdClaims struct {
	Fid string `json:"fid"`
	jwt.RegisteredClaims
}

// HanzoFilerClaims is created e.g. by S3 proxy server and consumed by Filer server.
// Right now, it only contains the standard claims; but this might be extended later
// for more fine-grained permissions.
type HanzoFilerClaims struct {
	AllowedPrefixes []string `json:"allowed_prefixes,omitempty"`
	AllowedMethods  []string `json:"allowed_methods,omitempty"`
	jwt.RegisteredClaims
}

// HanzoFilerAdminClaims is presented by callers of the filer's IAM gRPC
...
type HanzoFilerAdminClaims struct {
	jwt.RegisteredClaims
}
