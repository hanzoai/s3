...
func (s *FilerPolicyStore) StorePolicy(ctx context.Context, filerAddress string, name string, policy *PolicyDocument) error {
	...
	// Store in filer
	return s.withFilerClient(filerAddress, func(client filer_pb.HanzoFilerClient) error {
...
func (s *FilerPolicyStore) GetPolicy(ctx context.Context, filerAddress string, name string) (*PolicyDocument, error) {
	...
	var policyData []byte
	err := s.withFilerClient(filerAddress, func(client filer_pb.HanzoFilerClient) error {
...
func (s *FilerPolicyStore) withFilerClient(filerAddress string, fn func(client filer_pb.HanzoFilerClient) error) error {
	if filerAddress == "" {
		return fmt.Errorf("filer address is required for FilerPolicyStore")
	}

	// Use the pb.WithGrpcFilerClient helper similar to existing Hanzo code
...
func (s *FilerPolicyStore) deleteLegacyPolicyFileIfPresent(ctx context.Context, client filer_pb.HanzoFilerClient, policyName string) error {
...
func (s *FilerPolicyStore) savePolicyFile(ctx context.Context, client filer_pb.HanzoFilerClient, fileName string, content []byte) error {
	now := time.Now().Unix()
