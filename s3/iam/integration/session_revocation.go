...
// implementation so revocations propagate across `s3` instances.
...
// Hanzo-issued session id is a safe random token, an external caller
...
func (f *FilerSessionRevocationStore) Revoke(ctx context.Context, filerAddress string, entry *RevocationEntry) error {
	...
	return f.withFilerClient(filerAddress, func(client filer_pb.HanzoFilerClient) error {
...
func (f *FilerSessionRevocationStore) IsRevoked(ctx context.Context, filerAddress string, jti string) (bool, error) {
	...
	revoked := false
	err := f.withFilerClient(filerAddress, func(client filer_pb.HanzoFilerClient) error {
...
func (f *FilerSessionRevocationStore) withFilerClient(filerAddress string, fn func(filer_pb.HanzoFilerClient) error) error {
