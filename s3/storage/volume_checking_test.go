...
// .idx is rebuilt sorted by key — what s3 fix and other rebuilds emitted via
...
func TestCheckVolumeDataIntegritySortedIndex(t *testing.T) {
	...
	// Rebuild the .idx sorted ascending by key, the shape s3 fix's SaveToIdx
...
// (the on-disk state s3 fix left behind) must reload writable, not flip to
...
func TestVolumeLoadStaysWritableWithKeySortedIndex(t *testing.T) {
	...
	// Rewrite the .idx sorted by key — the shape old s3 fix produced.
...
// shape s3 fix's SaveToIdx emitted (needle_map.AscendingVisit) — so a test can
...
func rewriteIdxSortedByKey(t *testing.T, idxPath string) {
	...
	// — the order s3 fix's AscendingVisit emitted.
