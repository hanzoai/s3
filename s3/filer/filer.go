...
import (
	...
	"github.com/hanzoai/s3/s3/filer/empty_folder_cleanup"
...
func (f *Filer) MaybeBootstrapFromOnePeer(self pb.ServerAddress, existingNodes []*master_pb.ClusterNodeUpdate, snapshotTime time.Time) (err error) {
	...
	return pb.WithFilerClient(false, f.UniqueFilerId, pb.ServerAddress(earliestNode.Address), f.GrpcDialOption, func(client filer_pb.HanzoFilerClient) error {
...
//
//	if err := pb_filer_client.WithFilerClient(ctx, func(client filer_pb.HanzoFilerClient) error {
