...
type FoundationDBStore struct {
	...
	hanzoDir    directory.DirectorySubspace
...
func (store *FoundationDBStore) Initialize(configuration util.Configuration, prefix string) error {
	...
	configuration.SetDefault(prefix+"directory_prefix", "hanzo")
...
func (store *FoundationDBStore) initialize(clusterFile string, apiVersion int) error {
	...
	// Create/open hanzo directory
	store.hanzoDir, err = directory.CreateOrOpen(store.database, []string{store.directoryPrefix}, nil)
	if err != nil {
		return fmt.Errorf("failed to create/open hanzo directory: %w", err)
	}
...
func (store *FoundationDBStore) ListDirectoryPrefixedEntries(ctx context.Context, dirPath util.FullPath, startFileName string, includeStartFile bool, limit int64, prefix string, eachEntryFunc filer.ListEachEntryFunc) (lastFileName string, err error) {
	...
	dirRange, err := fdb.PrefixRange(store.hanzoDir.Pack(dirTuple))
	if err != nil {
	...
	if prefix != "" {
		...
		beginKey = store.hanzoDir.Pack(tuple.Tuple{string(dirPath), prefix})
		endKey = dirEnd

		// Use Strinc to get the next string for proper prefix range
		if nextPrefix, strincErr := fdb.Strinc([]byte(prefix)); strincErr == nil {
			endKey = store.hanzoDir.Pack(tuple.Tuple{string(dirPath), string(nextPrefix)})
		}
	} else {
	...
	if startFileName != "" {
		...
		startKey := store.hanzoDir.Pack(tuple.Tuple{string(dirPath), startFileName})
		if includeStartFile {
