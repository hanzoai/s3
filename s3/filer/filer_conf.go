...
func ReadFilerConfFromFilers(filerGrpcAddresses []pb.ServerAddress, grpcDialOption grpc.DialOption, masterClient *wdclient.MasterClient) (*FilerConf, error) {
	...
	if err := pb.WithOneOfGrpcFilerClients(false, filerGrpcAddresses, grpcDialOption, func(client filer_pb.HanzoFilerClient) error {
		if masterClient != nil {
			...
			if err := ReadEntry(masterClient, client, DirectoryEtcHanzo, FilerConfName, &buf); err != nil {
				return err
			}
			data = buf.Bytes()
			return nil
		}
		content, err := ReadInsideFiler(context.Background(), client, DirectoryEtcHanzo, FilerConfName)
		if err != nil {
			return err
		}
		data = content
		...
		return nil, fmt.Errorf("read %s/%s: %v", DirectoryEtcHanzo, FilerConfName, err)
	}

	fc := NewFilerConf()
	if len(data) > 0 {
		if err := fc.LoadFromBytes(data); err != nil {
			return nil, fmt.Errorf("parse %s/%s: %v", DirectoryEtcHanzo, FilerConfName, err)
		}
	}
	return fc, nil
}
...
func (fc *FilerConf) loadFromFiler(filer *Filer) (err error) {
	filerConfPath := util.NewFullPath(DirectoryEtcHanzo, FilerConfName)
