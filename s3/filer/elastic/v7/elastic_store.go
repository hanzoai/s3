...
func (store *Elastic8Store) Initialize(configuration s3_util.Configuration, prefix string) (err error) {
	store.es8 = true
	return store.ElasticStore.Initialize(configuration, prefix)
}

func (store *ElasticStore) Initialize(configuration s3_util.Configuration, prefix string) (err error) {
...
func (store *ElasticStore) ListDirectoryPrefixedEntries(ctx context.Context, dirPath s3_util.FullPath, startFileName string, includeStartFile bool, limit int64, prefix string, eachEntryFunc filer.ListEachEntryFunc) (lastFileName string, err error) {
...
func (store *ElasticStore) InsertEntry(ctx context.Context, entry *filer.Entry) (err error) {
	...
	id := s3_util.Md5String([]byte(entry.FullPath))
	esEntry := &ESEntry{
		ParentId: s3_util.Md5String([]byte(dir)),
		Entry:    entry,
	}
	if store.es8 {
		esEntry.Id = id
...
func (store *ElasticStore) FindEntry(ctx context.Context, fullpath s3_util.FullPath) (entry *filer.Entry, err error) {
	index := getIndex(fullpath, false)
	id := s3_util.Md5String([]byte(fullpath))
...
func (store *ElasticStore) DeleteEntry(ctx context.Context, fullpath s3_util.FullPath) (err error) {
	index := getIndex(fullpath, false)
	id := s3_util.Md5String([]byte(fullpath))
...
func (store *ElasticStore) DeleteFolderChildren(ctx context.Context, fullpath s3_util.FullPath) (err error) {
...
func (store *ElasticStore) ListDirectoryEntries(ctx context.Context, dirPath s3_util.FullPath, startFileName string, includeStartFile bool, limit int64, eachEntryFunc filer.ListEachEntryFunc) (lastFileName string, err error) {
...
func (store *ElasticStore) listDirectoryEntries(
	ctx context.Context, fullpath s3_util.FullPath, startFileName string, inclusive bool, limit int64, eachEntryFunc filer.ListEachEntryFunc) (lastFileName string, err error) {
	first := true
	index := getIndex(fullpath, true)
	nextStart := ""
	parentId := s3_util.Md5String([]byte(fullpath))
	...
	for {
		...
		if (startFileName == "" && first) || inclusive {
			...
			after := s3_util.Md5String([]byte(fullPath))
...
func getIndex(fullpath s3_util.FullPath, isDirectory bool) string {
