...
func (store *EtcdStore) Initialize(configuration s3_util.Configuration, prefix string) error {
...
func (store *EtcdStore) InsertEntry(ctx context.Context, entry *filer.Entry) (err error) {
	...
	if len(entry.GetChunks()) > filer.CountEntryChunksForGzip {
		meta = s3_util.MaybeGzipData(meta)
	}
...
func (store *EtcdStore) FindEntry(ctx context.Context, fullpath s3_util.FullPath) (entry *filer.Entry, err error) {
	...
	err = entry.DecodeAttributesAndChunks(s3_util.MaybeDecompressData(resp.Kvs[0].Value))
	if err != nil {
...
func (store *EtcdStore) DeleteEntry(ctx context.Context, fullpath s3_util.FullPath) (err error) {
	key := genKey(fullpath.DirAndName())
...
func (store *EtcdStore) DeleteFolderChildren(ctx context.Context, fullpath s3_util.FullPath) (err error) {
...
func (store *EtcdStore) ListDirectoryPrefixedEntries(ctx context.Context, dirPath s3_util.FullPath, startFileName string, includeStartFile bool, limit int64, prefix string, eachEntryFunc filer.ListEachEntryFunc) (lastFileName string, err error) {
	...
	for _, kv := range resp.Kvs {
		...
		if decodeErr := entry.DecodeAttributesAndChunks(s3_util.MaybeDecompressData(kv.Value)); decodeErr != nil {
			err = decodeErr
...
func (store *EtcdStore) ListDirectoryEntries(ctx context.Context, dirPath s3_util.FullPath, startFileName string, includeStartFile bool, limit int64, eachEntryFunc filer.ListEachEntryFunc) (lastFileName string, err error) {
