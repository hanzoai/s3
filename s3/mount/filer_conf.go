...
func (wfs *WFS) subscribeFilerConfEvents() (*meta_cache.MetadataFollower, error) {
	confDir := filer.DirectoryEtcHanzo
	confName := filer.FilerConfName
	confFullName := filepath.Join(filer.DirectoryEtcHanzo, filer.FilerConfName)

	// read current conf
	err := wfs.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
