...
func init() {
	...
	masterOptions.telemetryUrl = cmdServer.Flag.String("master.telemetry.url", "", "telemetry server URL to send usage statistics")
	...
	filerOptions.localSocket = cmdServer.Flag.String("filer.localSocket", "", "default to /tmp/hanzo-filer-<port>.sock")
	...
	sftpOptions.bannerMessage = cmdServer.Flag.String("sftp.bannerMessage", "Hanzo S3 SFTP Server - Unauthorized access is prohibited", "message displayed before authentication")
	...
	sftpOptions.localSocket = cmdServer.Flag.String("sftp.localSocket", "", "default to /tmp/hanzo-sftp-<port>.sock")
	...
	*volumeDataFolders = util.ResolveCommaSeparatedPaths(*volumeDataFolders)
	...
	for _, svc := range []struct {
		...
		if *svc.starting {
			...
			pb.RegisterLocalGrpcSocket(*serverIp, *svc.portGrpc, fmt.Sprintf("/tmp/hanzo-%s-grpc-%d.sock", svc.name, *svc.portGrpc))
		}
	}
