...
func printAutocompleteScript(shell string) bool {
	...
	switch shell {
	case "bash":
		fmt.Printf("complete -C %q s3\n", binPath)
	case "zsh":
		fmt.Printf("autoload -U +X bashcompinit && bashcompinit\n")
		fmt.Printf("complete -o nospace -C %q s3\n", binPath)
	case "fish":
		fmt.Printf(`function __complete_s3
    set -lx COMP_LINE (commandline -cp)
    test -z (commandline -ct)
    and set COMP_LINE "$COMP_LINE "
    %q
end
complete -f -c s3 -a "(__complete_s3)"
`, binPath)
	default:
...
func installAutoCompletion() bool {
	...
	err := completeinstall.Install("s3")
	if err != nil {
...
func uninstallAutoCompletion() bool {
	...
	err := completeinstall.Uninstall("s3")
	if err != nil {
...
Usage:
    s3 autocomplete [bash|zsh|fish]  # print autocomplete script to stdout
    s3 autocomplete install          # install to shell config files
    ...
    You can then add it to your shell configuration manually, e.g.:
        ...
        # For bash:
        s3 autocomplete bash >> ~/.bashrc

        # Or use eval in your shell config:
        eval "$(s3 autocomplete bash)"
...
var cmdUnautocomplete = &Command{
	...
	Long: `s3 autocomplete is uninstalled in the shell.
