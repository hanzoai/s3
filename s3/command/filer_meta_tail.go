...
var cmdFilerMetaTail = &Command{
	...
	s3 filer.meta.tail -timeAgo=30h | grep truncate
	s3 filer.meta.tail -timeAgo=30h | jq .
	s3 filer.meta.tail -timeAgo=30h -untilTimeAgo=20h | jq .
	s3 filer.meta.tail -timeAgo=30h | jq .eventNotification.newEntry.name

	s3 filer.meta.tail -timeAgo=30h -es=http://<elasticSearchServerHost>:<port> -es.index=hanzo
...
var (
	...
	esIndex     = cmdFilerMetaTail.Flag.String("es.index", "hanzo", "ES index name")
)
