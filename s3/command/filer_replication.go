...
import (
	...
	"github.com/hanzoai/s3/s3/replication"
	"github.com/hanzoai/s3/s3/replication/sink"
	"github.com/hanzoai/s3/s3/replication/sub"
...
var cmdFilerReplicate = &Command{
	...
	Run "s3 scaffold -config=replication" to generate a replication.toml file and customize the parameters.
...
func runFilerReplicate(cmd *Command, args []string) bool {
	...
	if notificationInput == nil {
		...
		println("Please follow 's3 scaffold -config=notification' to see example notification configurations.")
		return true
	}
