...
func (option *RemoteGatewayOptions) WithFilerClient(streamingMode bool, fn func(filer_pb.HanzoFilerClient) error) error {
	return pb.WithFilerClient(streamingMode, option.clientId, pb.ServerAddress(*option.filerAddress), option.grpcDialOption, func(client filer_pb.HanzoFilerClient) error {
		return fn(client)
	})
}
...
func init() {
	...
	remoteGatewayOptions.filerAddress = cmdFilerRemoteGateway.Flag.String("filer", "localhost:8888", "filer of the Hanzo S3 cluster")
...
var cmdFilerRemoteGateway = &Command{
	...
	If any bucket is created, deleted, or updated, it will mirror the changes to remote object store.
		...
		s3 filer.remote.gateway -createBucketAt=cloud1
...
func runFilerRemoteGateway(cmd *Command, args []string) bool {
	...
	// check buckets again
	remoteGatewayOptions.WithFilerClient(false, func(filerClient filer_pb.HanzoFilerClient) error {
