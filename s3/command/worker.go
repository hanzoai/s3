...
var cmdWorker = &Command{
	...
	Long: `Start an external plugin worker using s3/pb/plugin.proto over gRPC.
...
Examples:
  s3 worker -admin=localhost:23646
  s3 worker -admin=localhost:23646 -jobType=all
  s3 worker -admin=localhost:23646 -jobType=default
  s3 worker -admin=localhost:23646 -jobType=heavy
  s3 worker -admin=localhost:23646 -jobType=default,iceberg
  s3 worker -admin=localhost:23646 -jobType=vacuum,volume_balance
  s3 worker -admin=localhost:23646 -jobType=erasure_coding
  s3 worker -admin=admin.example.com:23646 -id=plugin-vacuum-a -heartbeat=10s
  s3 worker -admin=localhost:23646 -workingDir=/var/lib/hanzo-plugin
  s3 worker -admin=localhost:23646 -metricsPort=9327 -metricsIp=0.0.0.0
`,
}
