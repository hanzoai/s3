...
The backup writes to another filer store specified in a backup_filer.toml.
	...
	s3 filer.meta.backup -config=/path/to/backup_filer.toml -filer="localhost:8888"
	s3 filer.meta.backup -config=/path/to/backup_filer.toml -filer="localhost:8888" -restart
...
func (metaBackup *FilerMetaBackupOptions) WithFilerClient(streamingMode bool, fn func(filer_pb.HanzoFilerClient) error) error {

	return pb.WithFilerClient(streamingMode, metaBackup.clientId, pb.ServerAddress(*metaBackup.filerAddress), metaBackup.grpcDialOption, func(client filer_pb.HanzoFilerClient) error {
		return fn(client)
	})
