...
import (
	...
	_ "github.com/hanzoai/s3/s3/worker/tasks/balance"
	_ "github.com/hanzoai/s3/s3/worker/tasks/erasure_coding"
)
...
// s3 mini Ctrl+C, BEFORE filer/volume/master tear down. It is rebuilt on
...
// context.Background() if not running inside s3 mini.
...
Example Usage:
	s3 mini                   # Use current directory
	s3 mini -dir=/data        # Custom data directory
	s3 mini -dir=/data -master.port=9444  # Custom master port
	s3 mini -dir=/data -bucket=my-bucket             # Pre-create an S3 bucket on startup
	s3 mini -dir=/data -bucket=bucket1,bucket2       # Pre-create multiple S3 buckets
	s3 mini -dir=/data -tableBucket=iceberg-tables   # Pre-create an S3 Tables bucket
...
func initMiniFilerFlags() {
	...
	miniFilerOptions.localSocket = cmdMini.Flag.String("filer.localSocket", "", "default to /tmp/hanzo-filer-<port>.sock")
...
func initMiniS3Flags() {
	...
	miniS3Options.zapPort = cmdMini.Flag.Int("s3.zap.port", 0, "native ZAP service-mesh listen port for in-cloud callers (0 disables; external S3 clients use the HTTPS S3 API)")
	...
	miniS3Options.localSocket = cmdMini.Flag.String("s3.localSocket", "", "default to /tmp/hanzo-s3-<port>.sock")
...
func initMiniAdminFlags() {
	...
	miniAdminOptions.urlPrefix = cmdMini.Flag.String("admin.urlPrefix", "", "URL path prefix when running the admin UI behind a reverse proxy under a subdirectory (e.g. /hanzo)")
}
...
func runMini(cmd *Command, args []string) bool {
	...
	*miniAdminOptions.dataDir = util.ResolvePath(*miniAdminOptions.dataDir)
	...
	pb.RegisterLocalGrpcSocket(*miniIp, *miniMasterOptions.portGrpc, fmt.Sprintf("/tmp/hanzo-master-grpc-%d.sock", *miniMasterOptions.portGrpc))
	pb.RegisterLocalGrpcSocket(*miniIp, *miniOptions.v.portGrpc, fmt.Sprintf("/tmp/hanzo-volume-grpc-%d.sock", *miniOptions.v.portGrpc))
	pb.RegisterLocalGrpcSocket(*miniIp, *miniFilerOptions.portGrpc, fmt.Sprintf("/tmp/hanzo-filer-grpc-%d.sock", *miniFilerOptions.portGrpc))
	if *miniS3Options.portGrpc > 0 {
		pb.RegisterLocalGrpcSocket(*miniIp, *miniS3Options.portGrpc, fmt.Sprintf("/tmp/hanzo-s3-grpc-%d.sock", *miniS3Options.portGrpc))
	}
	pb.RegisterLocalGrpcSocket(*miniIp, *miniAdminOptions.grpcPort, fmt.Sprintf("/tmp/hanzo-admin-grpc-%d.sock", *miniAdminOptions.grpcPort))
...
func startMiniAdminWithWorker(allServicesReady chan struct{}) {
	...
	*miniAdminOptions.master = masterAddr
	...
	// Normalize URL prefix the same way `s3 admin` does.
...
func startMiniWorker(workerDir string) {
	...
	// Create admin client
	adminClient, err := worker.CreateAdminClient(adminAddr, workerInstance.ID())
	if err != nil {
...
func printWelcomeMessage() {
	...
	default:
		...
		sb.WriteString("    s3 mini -dir=/data\n")
...
func ensureMiniBuckets(bucketSpec string) error {
	...
	return pb.WithGrpcFilerClient(false, 0, filerAddress, grpcDialOption, func(client filer_pb.HanzoFilerClient) error {
