...
type FilerOptions struct {
	...
	// and by s3 mini; nil for standalone s3 filer.
...
func init() {
	...
	f.localSocket = cmdFiler.Flag.String("localSocket", "", "default to /tmp/hanzo-filer-<port>.sock")
	...
	filerS3Options.localSocket = cmdFiler.Flag.String("s3.localSocket", "", "default to /tmp/hanzo-s3-<port>.sock")
	...
	filerSftpOptions.bannerMessage = cmdFiler.Flag.String("sftp.bannerMessage", "Hanzo S3 SFTP Server - Unauthorized access is prohibited", "message displayed before authentication")
	...
	filerSftpOptions.localSocket = cmdFiler.Flag.String("sftp.localSocket", "", "default to /tmp/hanzo-sftp-<port>.sock")
}
...
func filerLongDesc() string {
	...
	The configuration file "filer.toml" is read from ".", "$HOME/.s3/", "/usr/local/etc/hanzo/", or "/etc/hanzo/", in that order.
	If the "filer.toml" is not found, an embedded filer store will be created under "-defaultStoreDir".

	The example filer.toml configuration file can be generated by "s3 scaffold -config=filer"
...
func runFiler(cmd *Command, args []string) bool {
	...
	*f.defaultLevelDbDirectory = util.ResolvePath(*f.defaultLevelDbDirectory)
	...
	// the admin UI and s3 shell talk to is wired up unconditionally, and it
...
func (fo *FilerOptions) startFiler() {
	...
	fs, nfs_err := s3server.NewFilerServer(defaultMux, publicVolumeMux, &s3server.FilerOption{
		Masters:                   fo.masters,
	...
	// zapServers collects every ZAP transport listener started below so shutdown
	// can close them all (closing releases the port for restart — important for
	// the start/stop/start cycle of s3 mini and integration tests).
	...
	if credentialManager != nil {
		...
		// transport.Serve starts its own accept loop and returns the *Server
		// handle; keep it so Close() can stop the listener on shutdown.
		if iamLocalL != nil {
			zapServers = append(zapServers, transport.Serve(iamLocalL, iamDispatch))
		}
		zapServers = append(zapServers, transport.Serve(iamL, iamDispatch))
	...
	stopZapServers := func() {
		glog.V(0).Infof("Stopping filer/IAM ZAP transport servers")
		for _, s := range zapServers {
			_ = s.Close()
		}
	}
	stopGrpcServer := stopZapServers

	var socketServer *http.Server
	if runtime.GOOS != "windows" {
		localSocket := *fo.localSocket
		if localSocket == "" {
			localSocket = fmt.Sprintf("/tmp/hanzo-filer-%d.sock", *fo.port)
		}
