...
func ensureBucketAllowEmptyFolders(ctx context.Context, filerClient filer_pb.FilerClient, mountRoot, bucketRootPath string) error {
	...
	if err := filerClient.WithFilerClient(false, func(client filer_pb.HanzoFilerClient) error {
...
func RunMount(option *MountOptions, umask os.FileMode) bool {
	...
	if *option.localSocket == "" {
		...
		*option.localSocket = fmt.Sprintf("/tmp/hanzo-mount-%d.sock", mountDirHash)
	}
	...
	hanzoFileSystem := mount.NewHanzoFileSystem(&mount.Option{
		MountDirectory:              dir,
	...
	if err = filer_pb.Mkdir(context.Background(), hanzoFileSystem, mountRootParent, mountDir, nil); err != nil {
		fmt.Printf("failed to create dir %s on filer %s: %v\n", mountRoot, filerAddresses, err)
		return false
	}
	if err := ensureBucketAllowEmptyFolders(context.Background(), hanzoFileSystem, mountRoot, bucketRootPath); err != nil {
		fmt.Printf("failed to set bucket auto-remove-empty-folders policy for %s: %v\n", mountRoot, err)
		return false
	}

	server, err := fuse.NewServer(hanzoFileSystem, dir, fuseMountOptions)
	if err != nil {
	...
	transport.Serve(montSocketListener, func(env []byte) ([]byte, error) {
		return mountwire.DispatchHanzoMount(hanzoFileSystem, env)
	})
	...
	hanzoFileSystem.WaitForAsyncFlush()

	hanzoFileSystem.ClearCacheDir()
