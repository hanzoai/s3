...
import (
	...
	"github.com/hanzoai/s3/s3/sftpd"
...
func init() {
	...
	sftpOptionsStandalone.bannerMessage = cmdSftp.Flag.String("bannerMessage", "Hanzo S3 SFTP Server - Unauthorized access is prohibited", "message displayed before authentication")
	...
	sftpOptionsStandalone.localSocket = cmdSftp.Flag.String("localSocket", "", "default to /tmp/hanzo-sftp-<port>.sock")
}
...
func (sftpOpt *SftpOptions) startSftpServer() bool {
	...
	for {
		err := pb.WithGrpcFilerClient(false, 0, filerAddress, grpcDialOption, func(client filer_pb.HanzoFilerClient) error {
	...
	if runtime.GOOS != "windows" {
		...
		if localSocket == "" {
			localSocket = fmt.Sprintf("/tmp/hanzo-sftp-%d.sock", *sftpOpt.port)
		}
