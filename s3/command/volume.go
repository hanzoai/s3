...
type VolumeServerOptions struct {
	...
	// ctx is cancelled. Used by integration tests and by s3 mini; nil for
	// standalone s3 volume.
...
func (v VolumeServerOptions) startVolumeServer(volumeFolders, maxVolumeCounts, volumeWhiteListOption string, minFreeSpaces []util.MinFreeSpace) {
	...
	volumeServer := s3server.NewVolumeServer(volumeMux, publicVolumeMux,
...
func (v VolumeServerOptions) startGrpcService(vs volume_server_pb.VolumeServerServer) *transport.Server {
	...
	var err error
	if tlsCfg := security.ServerTLSConfig(util.GetViper(), "grpc.volume"); tlsCfg != nil {
...
func (v VolumeServerOptions) startPublicHttpService(handler http.Handler) httpdown.Server {
	...
	glog.V(0).Infoln("Start Hanzo S3 volume server", version.Version(), "public at", publicListeningAddress)
...
func (v VolumeServerOptions) startClusterHttpService(handler http.Handler) (httpdown.Server, func()) {
	...
	glog.V(0).Infof("Start Hanzo S3 volume server %s at %s", version.Version(), listeningAddress)
