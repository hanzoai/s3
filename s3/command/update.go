...
import (
	...
	swv "github.com/hanzoai/s3/s3/util/version"
...
func init() {
	...
	updateOpt.dir = cmdUpdate.Flag.String("dir", filepath.Dir(path), "directory to save new s3.")
	updateOpt.name = cmdUpdate.Flag.String("name", name, "name of new s3. On windows, name shouldn't be same to the original name.")
	updateOpt.Version = cmdUpdate.Flag.String("version", "0", "specific version of s3 you want to download. If not specified, get the latest version.")
...
func runUpdate(cmd *Command, args []string) bool {
	...
	*updateOpt.dir = util.ResolvePath(*updateOpt.dir)
	...
	if runtime.GOOS == "windows" {
		if target == path {
			glog.Fatalf("On windows, name of the new s3 shouldn't be same to the original name.")
			return false
		}
	}

	glog.V(0).Infof("new s3 will be saved to %s", target)

	_, err := downloadRelease(context.Background(), target, *updateOpt.Version)
	if err != nil {
		glog.Errorf("unable to download s3: %v", err)
		return false
	}
	return true
}
...
func downloadRelease(ctx context.Context, target string, ver string) (version string, err error) {
	...
	if rel.Version == currentVersion {
		if ver == "0" {
			glog.V(0).Infof("s3 is up to date")
		} else {
			glog.V(0).Infof("no need to download the same version of s3 ")
		}
	...
	if err != nil {
		...
		glog.V(0).Infof("successfully updated s3 to version %v\n", rel.Version)
	}
