...
// Collector gathers telemetry data from a Hanzo cluster
// Only the leader master will send telemetry to avoid duplicates
type Collector struct {
	client       *Client
	topo         *topology.Topology
	cluster      *cluster.Cluster
	masterServer interface{} // Will be set to *s3server.MasterServer to access client tracking
	version      string
	os           string
}
...
// SetVersion sets the Hanzo version
